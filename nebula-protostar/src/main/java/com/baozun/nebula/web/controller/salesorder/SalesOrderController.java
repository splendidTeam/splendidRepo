...
public class SalesOrderController extends BaseController{
	...
	private final static String							DISTRIBUTION_MODE_ID	= "distributionModeId";
	...
	public Pagination<PtsSalesOrderCommand> orderListJson(
		...
		Pagination<PtsSalesOrderCommand> result = salesOrderManager.findOrderListByQueryMapWithPage(
				queryBean.getPage(),
				sorts,
	...
	public Object saveOrder(
		...
		try{
			...
			if (manualFlag != null && manualFlag){
				...
				subOrdinate = sdkOrderService.saveManualOrder(shoppingCartCommand, salesOrderCommand);
			}else{
				if (Validator.isNotNullOrEmpty(memberId)){
				...
				subOrdinate = sdkOrderService.saveOrder(shoppingCartCommand, salesOrderCommand, memComboIds);
			}
	...
	public Object removeShoppingCartLine(
		...
		if (manualFlag != null && manualFlag){
			@SuppressWarnings("unchecked")
			List<ShoppingCartLineCommand> lines = (List<ShoppingCartLineCommand>) request.getSession().getAttribute(
					Constants.MANUALBUY_SESSION_SHOPCART);
			...
			if (Validator.isNotNullOrEmpty(memberId)){
				...
				@SuppressWarnings("unchecked")
				List<ShoppingCartLineCommand> lines = (List<ShoppingCartLineCommand>) request.getSession().getAttribute(
						Constants.IMMEDIATELYBUY_SESSION_SHOPCART);
	...
	public OrderRefreshDataCommand refreshData(
		...
		if (Validator.isNotNullOrEmpty(codes) && Validator.isNotNullOrEmpty(shoppingCartCommand)){
			List<PromotionSKUDiscAMTBySetting> shareList = sdkPromotionCalculationShareToSKUManager.sharePromotionDiscountToEachLine(
					shoppingCartCommand,
					shoppingCartCommand.getCartPromotionBriefList());
