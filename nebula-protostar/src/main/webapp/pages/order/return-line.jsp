...
<script type="text/javascript"
	src="${base}/scripts/salesorder/return-line.js"></script>
...
<script type="text/javascript"
	src="${base}/scripts/salesorder/detail.js"></script>
...
<body>
	...
	<div class="content-box width-percent100">
		...
		<div class="ui-tag-change">
			<div class="tag-change-content" style="width: 100%;">
				<div style="float: left; margin-left: 10px;">
					...
					<div class="ui-block-line ui-block-line-37">
						<label><spring:message code="order.code" /></label> <label>
							${SoReturnApplication.soOrderCode }</label>
					</div>
					<div class="ui-block-line ui-block-line-37">
					...
					<div class="ui-block-line ui-block-line-37">
						<label> <spring:message
								code="order.return.rsneeded_returninvoice" /></label> <label>
							${SoReturnApplication.isNeededReturnInvoice } </label>
					</div>
						...
						<label> <spring:message code="order.refund.status" /></label>
						 <label>
							<c:if test="${SoReturnApplication.refundStatus == 0}">
							 			 ...
							 			 进行中
							 		 </c:if> <c:if test="${SoReturnApplication.refundStatus == 2}">
							 			 成功
							 		 </c:if> <c:if test="${SoReturnApplication.refundStatus == 3}">
							 			 失败
							 		 </c:if>
					...
					<div class="ui-block-line ui-block-line-37">
						<label> <spring:message code="order.refund.bank" /></label> <label>
							${SoReturnApplication.refundBank }</label>
					</div>
					<div class="ui-block-line ui-block-line-37">
					...
					<div class="ui-block-line ui-block-line-37">
						<label> <spring:message code="order.returnAddress" /></label> <label>
							${SoReturnApplication.returnAddress }</label>
					</div>

					<c:if test="${deliveryInfo != null}">

						<div class="ui-block-line ui-block-line-37">
							<label> <spring:message code="order.exchange.address" /></label>
							<label>
								${deliveryInfo.country}${deliveryInfo.province}${deliveryInfo.city}${deliveryInfo.district}${deliveryInfo.town}${deliveryInfo.address}</label>
						</div>

						<div class="ui-block-line ui-block-line-37">
							<label> <spring:message code="order.exchange.receiver" /></label>
							<label> ${deliveryInfo.receiver}</label>
						</div>

						<div class="ui-block-line ui-block-line-37">
							<label> <spring:message
									code="order.exchange.receiverphone" /></label> <label>
								${deliveryInfo.receiverPhone}</label>
						</div>

						<div class="ui-block-line ui-block-line-37">
							<label> <spring:message
									code="order.exchange.receivermobile" /></label> <label>
								${deliveryInfo.receiverMobile}</label>
						</div>

						<div class="ui-block-line ui-block-line-37">
							<label> <spring:message code="order.exchange.transName" /></label>
							<label> ${deliveryInfo.transName}</label>
						</div>

						<div class="ui-block-line ui-block-line-37">
							<label> <spring:message code="order.exchange.transCode" /></label>
							<label> ${deliveryInfo.transCode}</label>
						</div>

						<div class="ui-block-line ui-block-line-37">
							<label> <spring:message code="order.exchange.description" /></label>
					...
					<div class="ui-block-line ui-block-line-37">
						<label> <spring:message code="order.return.approver" /></label> <label>
							<div>${SoReturnApplication.approver }</div>
						</label>
					</div>
					<div class="ui-block-line ui-block-line-37">
						<label> <spring:message code="order.approve.time" /></label> <label>
							<fmt:formatDate value="${SoReturnApplication.approveTime}"
								type="both" />
						</label>
					</div>
					...
					<div class="ui-block-line ui-block-line-37">
						...
						<label> <fmt:formatDate
								value="${SoReturnApplication.createTime}" type="both" />
						</label>
					</div>
				</div>
			</div>
		...
		<!-- 退货商品清单begin -->
		<div class="ui-block-title1" style="color: black; background: white;">
			<spring:message code="order.returnLine" />
		</div>
		<div class="ui-tag-change">
			<div class="tag-change-content">
				<div class="border-grey ui-loxia-simple-table"
					caption="<spring:message code='order.returnLine'/>">
					<table cellspacing="0" cellpadding="0">
						...
						<c:if test="${empty SoReturnLine}">
							<tr>
								<td colspan="12" style="text-align: center; color: red;"><spring:message
										code="order.return.line.notexist" /></td>
							</tr>
						</c:if>
					</table>
