...
<script type="text/javascript" src="${base}/scripts/product/item/send-instation-message.js"></script>
</head>
<body>
	<div class="content-box width-percent100">
		<div class="ui-block">
			<div class="ui-title1 title_unchecked">
				<img src="${base}/images/wmi/blacks/32x32/tag.png">发送站内信
			</div>
					...
					<input type="hidden" class="bt">
						<div class="tag-change-in">
								...
								<table>
									<tr>
										<td><input style="margin-right: 0px;" type="radio"
											...
											style="margin-right: 0px;" type="radio" name="radio"
											value="2">邮箱 <input style="margin-right: 0px;"
											type="radio" name="radio" value="3">用户名 &nbsp;&nbsp;<span
											style="color: red;">(多个以英文逗号分隔)</span></td>
									</tr>
									<tr>
									</tr>
									<tr>
										<textarea id="text" style="width: 100%; height: 300px;"
											rows="25" cols="200" name="massMeber"></textarea>
									</tr>
								</table>
							</div>
							<!-- 发送模板 -->
							<div class=""
								...
								<div class="">
									<span> 发送站内信模板类型: <select name="q_int_lifeCycle"
										class="ui-loxia-default ui-corner-all messagethp"
										aria-disabled="false">
											...
											<c:forEach var="templatelist" items="${templatelist}">
												<option value="${templatelist.id}">${templatelist.title}</option>
													...
													<%-- <span id="tip" style="margin: 0px 0px 0px 30px;display: none;">
									     模板发送类型:
								         <select id="lifeCycle" name="q_int_lifeCycle" class="ui-loxia-default ui-corner-all" aria-disabled="false">
								             <option value="">请选择</option>
								             <option value="">模板发送</option>
					                         <option value="1">短信发送</option>
					                         <option value="0">邮件发送</option>
					                     </select>
					                 </span> --%>
								</div>
								<div style="text-align: right;">
									<input type="button" value="发送信息" title="发送信息"
										class="button orange confirmsendtext" />
								</div>
							</div>
							<!-- 结束-->
						</div>
						<div class="tag-change-in">
							...
							<div class=""
								style="padding: 10px; border: 1px solid #DEDEDE; clear: both; display: block; overflow: hidden;">
								<div class="">
									<span> 发送站内信模板类型: <select id="messagethp"
										name="q_int_lifeCycle" class="ui-loxia-default ui-corner-all  usersetptype"
										aria-disabled="false">
											<option value="0">请选择</option>
											<c:forEach var="templatelist" items="${templatelist}">
												<option value="${templatelist.id}">${templatelist.title}</option>
				         ...
				         <select id="lifeCycle" name="q_int_lifeCycle" class="ui-loxia-default ui-corner-all" aria-disabled="false">
	                         ...
	                         <option value="0">邮件发送</option>
								...
								<div style="text-align: right;">
									<input type="button" value="发送信息" title="发送信息"
										class="button orange confirmsenduserid" />
								</div>
							</div>
							<!-- 公共的 -->
							<!-- 结束-->
