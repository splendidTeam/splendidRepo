...
<div class="tag-change-content">
            <div class="tag-change-in block">
                    <div class="ui-block">
                    <div class="ui-block-title1"><spring:message code='item.searchCodition.editCoditionItem'/></div>
<form id="submitForm1" method="post" action="/i18n/item/itemSearchCondition/managerSaveConditionItem.htm">
<input id="id" type="hidden" value="${searchConditionItemVo.id}" name="searchConditionItem.id" />
<input id="pid" type="hidden" value="${searchConditionVo.id}" name="pid" />
  		...
  		<div class="ui-block-line">
	    	...
	    	<select id="propertyValueId" name="searchConditionItem.propertyValueId" loxiaType="select" mandatory="true" checkmaster="checkSelect"/>
	    		...
	    		<c:forEach var="propertyValue" items="${propertyValueList}">
	    			<option <c:if test="${searchConditionItemVo.propertyValueId==propertyValue.id}">selected='selected'</c:if> value="${propertyValue.id}">${propertyValue.value}</option>
	    		</c:forEach>
	    	</select>
	  	</div>
  	</c:when>
  	<c:otherwise>
		<input id="coditionId" type="hidden" value="${searchConditionVo.id}" name="searchConditionItem.coditionId" />
  		<div class="ui-block-line">
	    	...
	    	<input type="text" value="${searchConditionItemVo.areaMin}" loxiaType="number" name="searchConditionItem.areaMin" id="areaMin" placeholder="<spring:message code='item.searchCodition.min'/>" mandatory="true">
	  	</div>
	  	<div class="ui-block-line">
	    	<label><spring:message code='item.searchCodition.max'/></label>
	    	<input type="text" value="${searchConditionItemVo.areaMax}" loxiaType="number" name="searchConditionItem.areaMax" id="areaMax" placeholder="<spring:message code='item.searchCodition.max'/>" mandatory="true">
	  	</div>
  	</c:otherwise>
  </c:choose>
  ...
  <c:if test="${i18nOnOff==true}">
	  ...
	  <div class="ui-block-line">
	    ...
	    <input type="text" value="${searchConditionItemVo.name.values[i]}" loxiaType="input" name="searchConditionItem.name.values[${status.index}]" id="name" placeholder="<spring:message code='item.searchCodition.itemName'/>" mandatory="true">
	  	 <c:if test="${not empty searchConditionItemVo.name.langs}">
	  	 <input type="text"  class="i18n-lang" loxiaType="input" name="searchConditionItem.name.langs[${status.index}]" value="${searchConditionItemVo.name.langs[i]}">
	  	 </c:if>
	  	 <c:if test="${empty searchConditionItemVo.name.langs}">
  ...
  <div class="ui-block-line">
    <label><spring:message code='item.searchCodition.itemName'/></label>
    <input type="text" value="${searchConditionItemVo.name.value}" loxiaType="input" name="searchConditionItem.name.value" id="name" placeholder="<spring:message code='item.searchCodition.itemName'/>" mandatory="true">
  </div>
  </c:if>
  
  <div class="ui-block-line">
    <label><spring:message code='item.searchCodition.sort'/></label>
    <input type="text" value="${searchConditionItemVo.sort}" loxiaType="number" name="searchConditionItem.sort" id="sort" placeholder="<spring:message code='item.searchCodition.sort'/>" mandatory="true">
  </div>
  ...
  <div class="button-line">
		...
		<input type="button" id="canel" title="<spring:message code='item.searchCodition.canel'/>" value="<spring:message code='item.searchCodition.canel'/>" class="button goBackBtn"> 
  </div>
