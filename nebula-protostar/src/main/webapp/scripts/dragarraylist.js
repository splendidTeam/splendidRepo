	...
	$.fn.dragarraylist = function(obj){
			...
			var isAdd = o.add;
				...
				$j(strli).appendTo(self).on("click",function(){
					...
					if(nosignlength>0){
						...
						$j(this).find("em").on("click",function(e){
							...
							nps.confirm("是否删除此项","确认删除", function(){
				...
				if(newarray.length==self.find("li").length){
					...
					$j(".ui-block .ui-block-line").eq(0).find(".ui-loxia-default").focus();
					...
					$j(".ui-block .ui-block-line").eq(0).find(".ui-loxia-default").val("");
				}
