$j.extend(loxia.regional['zh-CN'],{
	"PROPERT_OPERATOR_TIP":"属性提示信息",
	 "LABEL_MEMBER_USERPIC":"会员头像",
	    ...
	    "INFO_SUCCESS":"请选择派发优惠券的类型",
	    "INFO_NULL":"请输入文本框的值!",
	    "INFO_TEMPLATE":"请输发送模板类型!",
	    "CHECK_USER":"请选择用户",
...
/*function checktype(type){
	if(type==1){
		...
		$j("#sem").css("display","none");
	}else if (type==3){
		$j("#sem").css("display","inline-block");
	   ...
	   /* 
	    $j("#box").click(function(){
				if($j(this).is(':checked')){
					$j("#em").css("display","inline-block");
				}else{
					$j("#em").css("display","none");
					if($j("#em").is(":visible")==false){
						$j("#tip").css("display","none");
						$j("#sem").css("display","none");
					}
				}
	    ...
	    $j("#boxc").click(function(){
			if($j(this).is(':checked')){
				$j("#tipc").css("display","inline-block");
			}else{ 
				$j("#tipc").css("display","none");
			}
	    });
	    
        
		
		var URL="/coupon/saveCouponSendMessage.json";
	    
	    
	  //派发优惠券
	    $j(".sendcoupon").click(function(){
	    	if (  $j("#text").val()==null ||$j("#text").val()=="") {
				...
				return;
			}else if($j("#coupontype").find("option:selected").val() == 0){
				nps.info(nps.i18n("INFO_TITLE_DATA"), nps.i18n("INFO_SUCCESS"));
				return;
			}else if ($j("input[id='box']:checked").is(':checked')){
				if($j("#messagetemplate").find("option:selected").val() == 0){
					nps.info(nps.i18n("INFO_TITLE_DATA"), nps.i18n("INFO_TEMPLATE"));
					return;
				}
			}
	    	var textvalue=$j("#text").val()==null ||$j("#text").val();
	    	
	    	var type=$j("input[type='radio']:checked").val();

	    	var coupontype=$j("#coupontype").find("option:selected").val();
	    	
	    	var tmpid=$j("#messagetemplate").find("option:selected").val();
	    	
	    	
	    	
	    	checktype(textvalue, type, function(ret){
				if(ret==""){
					var JSON={type:type,textvalue:textvalue,tmpid:tmpid,coupontype:coupontype};
			    	ajaxjosn(URL, JSON);
			    }else{
	    ...
	    $j(".usersendcoupon").click(function(){
	    	if ($j("input:checkbox[name='chedkId']:checked").length == 0) {
				nps.info(nps.i18n("INFO_TITLE_DATA"), nps.i18n("CHECK_USER"));
				return;
			}else if($j("#usercoupontype").find("option:selected").val() == 0){
				nps.info(nps.i18n("INFO_TITLE_DATA"), nps.i18n("INFO_SUCCESS"));
				return;
			}else if ($j("input[id='boxc']:checked").is(':checked')){
				if($j("#usermessagetemplate").find("option:selected").val() == 0){
					nps.info(nps.i18n("INFO_TITLE_DATA"), nps.i18n("INFO_TEMPLATE"));
					return;
				}
			}
	    	...
	    	var coupontype=$j("#usercoupontype").find("option:selected").val();
	    	
	    	
	    	var tmpid=$j("#usermessagetemplate").find("option:selected").val();
	    	
	    	
	    	var JSON={type:1,textvalue:userid,tmpid:tmpid,coupontype:coupontype};
