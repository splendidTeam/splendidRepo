...
var UPLOAD_URL = base + '/sku/skuUploadUpdatePrice.json;jsessionid=';
var itemErrorIds;
var skuErrorIds;
$j(document).ready(function(){
   ...
   //解决uploadify对于火狐浏览器的丢失session
	...
	$j("#sku-upload").uploadify({
		...
		'onUploadSuccess' 	: function(file, data, response) { 
			...
			if(result.isSuccess){
				...
				html+="错误编码如下:"+"</br>";
				$j.each(result.itemErrorIds,function(idx,obj){
					html+=obj+"</br>";
				});
				
				$j.each(result.skuErrorIds,function(idx,obj){
					html+=obj+"</br>";
				});
    ...
    $j("#downLoadTmplOfSkuInfo").on("click",function(){
		location.href="/item/skuItem-list.xlsx";
	});
