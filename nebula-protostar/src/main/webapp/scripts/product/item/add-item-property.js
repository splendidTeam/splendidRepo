...
function itTable(array, i, array1, str, properties, propertiesName) {
	if (array[i + 1] != null) {
		for (var k = 0; k < array1[0].length; k++) {
			str += "<td style='width:120px'>" + array1[0][k] + "</td>";
			// json
			properties += "{'id':'" + array1[1][k] + "','pId':'" + array1[3][k] + "'},";
			propertiesName += "{'id':'" + array1[1][k] + "','pId':'" + array1[3][k] + "','value':'" + array1[0][k]
					+ "','pName':'" + array1[2][k] + "'},";
			if (k > 0) {
				str = str.replace("<td style='width:120px'>" + array1[0][k - 1] + "</td>", "");
				// json
		...
		for (var j = 0; j < array1[0].length; j++) {
			...
			tr1.innerHTML = str
					...
					+ array1[0][j]
					+ "</td><td style='width:120px'><input type='text'  loxiaType='input'  name='codes' value=''/></td>"
					+ "<td style='width:120px'><input type='text' loxiaType='number' decimal='2' id='salePrices' name='salePrices' value=''/></td>"
					+ "<td style='width:120px'><input type='text'  loxiaType='number' decimal='2' id='listPrices' name='listPrices' value=''/></td>";
			// json
			var p = "'properties':'[" + properties + "{'id':'" + array1[1][j] + "','pId':'" + array1[3][j] + "'}]'";
			var pn = "'propertiesName':'[" + propertiesName + "{'id':'" + array1[1][j] + "','pId':'" + array1[3][j]
					+ "','value':'" + array1[0][j] + "','pName':'" + array1[2][j] + "'}]'";
...
function produceExtension() {
	...
	$j("#exten").css({'display': 'block', 'overflow-x':'auto', 'width':min_width});
	if (propertyArray.length > 0) { // 如果有 销售属性
		...
		if (propertyValueArray.length != propertyArray.length) {
			...
			htmlA = "";
			for (var j = 0; j < propertyNamesArray.length; j++) {
				htmlA += "<td style='width:120px'>" + propertyNamesArray[j] + "</td>";
			}
			html = "<tr>" + htmlA + "<td style='width:120px'>" + nps.i18n("MERCHANT_CODING") + "</td>"
					+ "<td style='width:120px'>" + nps.i18n("MERCHANT_SALEPRICE") + "</td>"
					+ "<td style='width:120px'>" + nps.i18n("MERCHANT_LISTPRICE") + "</td>" + "</tr>";
			$j("#extensionTable").html(html);
			list = "";
		...
		html = "<tr>" + "<td style='width:120px'>" + nps.i18n("MERCHANT_CODING") + "</td>" + "<td style='width:120px'>"
				+ nps.i18n("MERCHANT_SALEPRICE") + "</td>" + "<td style='width:120px'>"
				+ nps.i18n("MERCHANT_LISTPRICE") + "</td>" + "</tr>";
		html += "<td style='width:120px'><input type='text' mandatory='true' loxiaType='input'  name='codes' value=''/></td><td style='width:120px'><input type='text' mandatory='true' loxiaType='number' decimal='2' id='salePrices' name='salePrices' value=''/></td><td style='width:120px'><input type='text'  loxiaType='number' decimal='2' id='listPrices' name='listPrices' value=''/></td>";
		$j("#extensionTable").html(html);
				...
				if (lang != defaultlang) {
					me.find("textarea[propertyId='" + propertyId + "'][editingtype='5']").each(function() {
						if ($j(this).val() != "") {
							...
							if (inputValueArray.length > 0) {
								/*
								 * propertyIdArray[propertyValueArrayIndex] =
								 * propertyId;
								 * propertyValueArray[propertyValueArrayIndex] =
								 * inputValueArray;
								 * propertyValueInputArray[propertyValueArrayIndex] =
								 * valueArray; propertyValueArrayIndex++;
								 */
			...
			if (type == 1) {
				$j.each(propertyValueArray, function(j) {
					html += "<div class='priDiv'><span class='children-store'>"
							...
							+ "pvId='" + propertyValueArray[j].id + "' propertyId='" + property.id + "' " + "pvValue='"
...
function initProperties(industryId) {
	...
	var _data = loxia.syncXhr(base + '/item/findDynamicPropertisJson.json', json, {
		type : "GET"
	});
	...
	var backWarnEntity = loxia.syncXhr(base + '/item/findDynamicPropertis.json', json, {
		type : "GET"
	});
	if (backWarnEntity.isSuccess) {
		// 清空
										...
										if (jsonArray[i].property.editingType == 4 && i18nLang.key == defaultlang) {
											...
											if (jsonArray2 != null && jsonArray2.length > 0) {
														...
														.each(
																...
																function(j, val) {
																	if (jsonArray[i].property.isColorProp
																			&& jsonArray[i].property.hasThumb) {
																		picUrl = "<img src='" + baseUrl
																				+ "/images/1.png'>";
																	}
																	html1 += "<div class='priDiv'><span class='children-store'>"
																			...
																			+ jsonArray2[j].id
																			+ "' propertyId='"
																			+ jsonArray[i].property.id
																			+ "' "
																			+ "pvValue='"
																			+ jsonArray2[j].value
																			+ "' propertyName='"
																			+ jsonArray[i].property.name
																			+ "'"
																			+ "value='"
																			+ jsonArray2[j].id
																			+ "'>"
												...
												if (jsonProGroupArray != null && jsonProGroupArray.length > 0) {
													html3 = '<br><select  onchange="changeProGroup(this,1)" loxiaType="select"  propertyId="'
															+ jsonArray[i].property.id + '">';
													html3 += '<option value="">'
															+ nps.i18n("PLEASE_SELECT_PROPERTY_GROUP") + '</option>';
													$j.each(jsonProGroupArray, function(i) {
														html3 += '<option value="' + jsonProGroupArray[i].id + '">'
																+ jsonProGroupArray[i].name + '</option>';
													});
													html3 += "</select>"
												}

												html2 = "<div class='ui-block-line '><label style=''>"
														+ jsonArray[i].property.name
														+ "</label><div >"
														+ "<input type='hidden' value='"
														+ jsonArray[i].property.id
														+ "'  name = 'propertyIds' />"
														+ "<input type='hidden' value=''  name='propertyValueInputIds' pid='"
														+ jsonArray[i].property.id + "' />" + html3
														+ "<div class='wl-right-auto width-percent50 mt10'>" + html1
														+ " </div>" + " </div>" + " </div>";
											}
											...
											html2 = '<div class="ui-block-line "><label>' + jsonArray[i].property.name
													+ '</label>';
											html2 += "<input type='hidden' name = 'propertyIds' value='"
													+ jsonArray[i].property.id + "' />";
											html2 += "<input type='hidden' value='' class='propertyValueInputs' name='propertyValueInputs' pid='"
													+ jsonArray[i].property.id + "' />";
											html2 += "<input type='hidden' value='" + i18nLang.key
													+ "'  name='propertyValueInputs'/>";
											html2 += '<div class="priDiv">';
											if (jsonArray[i].property.isColorProp) {
												html2 += '<textarea class="customerSelect spTa" loxiaType="input" name="propertyValue" editingType="5" isColorProp="true" propertyId="'
														+ jsonArray[i].property.id
														+ '" style="width: 600px; height: 45px"  ></textarea>';
											} else {
												html2 += '<textarea class="customerSelect spTa" loxiaType="input" name="propertyValue" editingType="5" isColorProp="false" propertyId="'
														+ jsonArray[i].property.id
														+ '" style="width: 600px; height: 45px"  ></textarea>';

											}
											html2 += '<span style="margin: 17px 0 0 10px; position: absolute;">'
													+ i18nLang.value + '&nbsp;&nbsp;属性名之间请以"||"分隔</span>'
													+ '</div></div>';
										}
										...
										if (jsonArray[i].property.isColorProp) {
											...
											if (jsonArray[i].property.editingType == 4) {
												...
												if (propertyValueListJson != '' && propertyValueListJson.length > 0) {
													str = '<label>' + nps.i18n('IMAGE_SELECT_COLOR_PROPERTY')
															+ '</label>';
													...
													$j.each(propertyValueListJson, function(index, propertyValue) {
														str += '<option value="' + propertyValue.id + '#">'
								...
								if (jsonArray[i].property.isSaleProp) {
									...
									if (jsonArray[i].property.editingType == 4) {
										...
										if (jsonArray2 != null && jsonArray2.length > 0) {
											...
											propertyArray.push(jsonArray[i].property.id);
											propertyNameArray.push(jsonArray[i].property.name);
											// propSelectHtml+='<option
											// value="'+jsonArray[i].property.name+'"
											// }>'+jsonArray[i].property.name+'</option>';

											// 属性值分组处理
											if (jsonProGroupArray != null && jsonProGroupArray.length > 0) {
												html3 = '<br><select  onchange="changeProGroup(this,1)" loxiaType="select"  propertyId="'
														+ jsonArray[i].property.id + '">';
												html3 += '<option value="">' + nps.i18n("PLEASE_SELECT_PROPERTY_GROUP")
														+ '</option>';
												$j.each(jsonProGroupArray, function(i) {
													html3 += '<option value="' + jsonProGroupArray[i].id + '">'
															+ jsonProGroupArray[i].name + '</option>';
										...
										html2 += "<input type='hidden' name = 'propertyIds' value='"
												+ jsonArray[i].property.id + "' />";
										html2 += "<input type='hidden' value='' name='propertyValueInputs' pid='"
												+ jsonArray[i].property.id + "' />";
										html2 += '<div class="priDiv">';
										if (jsonArray[i].property.isColorProp) {
											html2 += '<textarea class="customerSelect spTa" loxiaType="input" name="propertyValue" editingType="5" isColorProp="true" propertyId="'
							...
							if (!jsonArray[i].property.isSaleProp) {
								...
								if (jsonArray[i].property.editingType == 1) {
									...
									if (i18nOnOff) {
										if (jsonArray[i].property.valueType == 1) {
											for (var j = 0; j < i18nLangs.length; j++) {
												var i18nLang = i18nLangs[j];
												inputHtml = inputHtml
														+ "<input type=\"text\" name=\"iProperties.propertyValue.values["
														+ num + "-" + j
														+ "]\" style='width: 600px' loxiaType=\"input\" " + required
														+ " />";
												inputHtml = inputHtml + '<input class="i18n-lang"  type="text" '
														+ 'name="iProperties.propertyValue.langs[' + num + '-' + j
														+ ']"  value="' + i18nLang.key + '"/><span>' + i18nLang.value
														+ '</span><br>';
											}
										}
										if (jsonArray[i].property.valueType == 2) {
											for (var j = 0; j < i18nLangs.length; j++) {
												...
												inputHtml = inputHtml
														+ "<input type=\"text\" name=\"iProperties.propertyValue.values["
														+ num + "-" + j + "]\" loxiaType=\"number\" " + required
														+ " />";
												inputHtml = inputHtml + '<input class="i18n-lang"  type="text" '
														+ 'name="iProperties.propertyValue.langs[' + num + '-' + j
														+ ']"  value="' + i18nLang.key + '"/><span>' + i18nLang.value
										...
										if (jsonArray[i].property.valueType == 4) {
											for (var j = 0; j < i18nLangs.length; j++) {
												var i18nLang = i18nLangs[j];
												inputHtml = inputHtml
														+ "<input type=\"text\" showtime=\"true\"   name=\"iProperties.propertyValue.values["
														+ num + "-" + j + "]\" loxiaType=\"date\" " + required + " />";
												inputHtml = inputHtml + '<input class="i18n-lang"  type="text" '
														+ 'name="iProperties.propertyValue.langs[' + num + '-' + j
														+ ']"  value="' + i18nLang.key + '"/><span>' + i18nLang.value
										...
										if (jsonArray[i].property.valueType == 1) {
											inputHtml = "<input type=\"text\" name=\"iProperties.propertyValue.value["
													+ num + "]\" style='width: 600px' loxiaType=\"input\" " + required
													+ " />";
										}
										if (jsonArray[i].property.valueType == 2) {
											inputHtml = "<input type=\"text\" name=\"iProperties.propertyValue.value["
													+ num + "]\" loxiaType=\"number\" " + required + " />";
										}
										if (jsonArray[i].property.valueType == 3) {
											inputHtml = "<input type=\"text\"   name=\"iProperties.propertyValue.value["
													+ num + "]\" loxiaType=\"date\" " + required + " />";
										}
										if (jsonArray[i].property.valueType == 4) {
											inputHtml = "<input type=\"text\" showtime=\"true\"   name=\"iProperties.propertyValue.value["
													+ num + "]\" loxiaType=\"date\" " + required + " />";
										}

									}

									html3 = "<label >" + jsonArray[i].property.name + "</label>" + "<div >" + inputHtml
											+ "<input type='hidden' name='iProperties.propertyId' value='"
											+ jsonArray[i].property.id + "'/>"
											...
											+ "<input type='hidden' name='iProperties.picUrl' value=''/>" + "</div>";
								} else if (jsonArray[i].property.editingType == 2) {
									html3 = "<label >"
											+ jsonArray[i].property.name
											+ "</label><div ><select style='width:160px;height:25px' onchange='doOther(this,"
											+ jsonArray[i].property.id + ")' name='iProperties.propertyValueId'>";
									$j.each(jsonArray2, function(j, val) {
										html3 += "<option value ='" + jsonArray2[j].id + "' >" + jsonArray2[j].value
												+ "</option>";
									});
									...
									if (i18nOnOff) {
										for (var j = 0; j < i18nLangs.length; j++) {
											var i18nLang = i18nLangs[j];
											html3 = html3 + "&nbsp;<input type='hidden' id='pv_"
													+ jsonArray[i].property.id
													+ "'  name='iProperties.propertyValue.values[" + num + "-" + j
													+ "]' value=''/>";
										...
										html3 = html3 + "&nbsp;<input type='hidden' id='pv_" + jsonArray[i].property.id
									...
									html3 = "<label >"
											+ jsonArray[i].property.name
											+ "</label><div ><select style='width:160px;height:25px' name='iProperties.propertyValueId'>";
									$j.each(jsonArray2, function(j, val) {
										html3 += "<option value ='" + jsonArray2[j].id + "'>" + jsonArray2[j].value
												+ "</option>";
									});
									html3 += "</select>" + "<input type='hidden' name='iProperties.propertyId' value='"
											+ jsonArray[i].property.id + "'/>"
											+ "<input type='hidden' name='iProperties.id' value=''/>"
											+ "<input type='hidden' name='iProperties.propertyDisplayValue' value=''/>"
											+ "<input type='hidden' name='iProperties.createTime' value=''/>"
									...
									if (i18nOnOff) {
										for (var j = 0; j < i18nLangs.length; j++) {
											var i18nLang = i18nLangs[j];
											html3 = html3
													+ "<input type='hidden' name='iProperties.propertyValue.values["
													+ num + "-" + j + "]' value=''/></div>";
											html3 = html3 + '<input class="i18n-lang"  type="text" '
													+ 'name="iProperties.propertyValue.langs[' + num + '-' + j
													+ ']"  value="' + i18nLang.key + '"/>';
										...
										html3 = html3 + "<input type='hidden' name='iProperties.propertyValue.value["
												+ num + "]' value=''/></div>";
									}
									...
									if (jsonArray2 != null && jsonArray2.length > 0) {
										...
										// 属性值分组处理
										if (jsonProGroupArray != null && jsonProGroupArray.length > 0) {
											html3 += '<br><select  onchange="changeProGroup(this,2)" loxiaType="select"  propertyId="'
													+ jsonArray[i].property.id + '">';
											html3 += '<option value="">' + nps.i18n("PLEASE_SELECT_PROPERTY_GROUP")
													+ '</option>';
											$j.each(jsonProGroupArray, function(i) {
										...
										$j
												.each(
														jsonArray2,
														function(j, val) {
															html3 += "<span class='children-store normalCheckBoxSpan'><input type='checkbox' class = 'normalCheckBoxCls' pid="
																	+ jsonArray[i].property.id
																	+ " mustCheck='"
																	+ jsonArray[i].property.name
																	+ "' name='' "
									...
									if (i18nOnOff) {
										for (var j = 0; j < i18nLangs.length; j++) {
											var i18nLang = i18nLangs[j];
											html3 = html3
													+ '<textarea loxiaType="input" name="iProperties.propertyValue.values['
													+ num + '-' + j
													+ ']"  style="width: 600px; height: 45px" ></textarea>';
											html3 = html3
													+ '<input class="i18n-lang"  type="text" name="iProperties.propertyValue.langs['
													+ num + '-' + j + ']"  value="' + i18nLang.key + '"/>';
											html3 = html3 + '<span style="margin: 17px 0 0 10px; position: absolute;">'
													+ i18nLang.value + '&nbsp;&nbsp;属性名之间请以"||"分隔</span><br>';
										}
									} else {
										html3 = html3
												+ '<textarea loxiaType="input" name="iProperties.propertyValue"  style="width: 600px; height: 45px" ></textarea>';
										html3 = html3
												+ '<span style="margin: 17px 0 0 10px; position: absolute;">属性名之间请以"||"分隔</span>';
									}
									html3 = html3
											+ '<input type="hidden" name="iProperties.propertyId" value="'
											+ jsonArray[i].property.id
		...
		$j.each(jsonArray, function(i, val) {
			if (!jsonArray[i].property.isSaleProp) {
				...
				if (jsonArray[i].property.editingType == 4) {
					if (jsonArray2 != null && jsonArray2.length > 0) {
						$j.each(jsonArray2, function(j, val) {
							rhtml3 += "<span class='hidBoxSpan'><div type='hidden' class = 'repNormalCheckBoxCls' pid="
									+ jsonArray[i].property.id + " pvid='" + jsonArray2[j].id + "'/></span>";
						});
					}
				}
			}
