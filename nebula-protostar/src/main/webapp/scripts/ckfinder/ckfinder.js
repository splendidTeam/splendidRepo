...
(function($j) {
	...
	a.ajax = (function() {
		var k = function() {
			...
			if (!s)
				...
				u.open('POST', p, t);
				u.setRequestHeader("X-CSRF-TOKEN", tokenValue);
			if (t)
										...
										.on(
												...
												function S(Q) {
													if (Q.data.space == 'mainMiddle') {
														...
														if (!i)
															R = x[0] + x[3];
														Q.data.html += "<div id='files_view' class='view ib files_thumbnails' aria-live='polite' role='main' tabindex='0' aria-controls='status_view'><h4 style='display:none;' class='message_content'></h4><div class='files_thumbnails fake no_list' role='list'></div>"
																+ R + '</div>';
													}
												});
								N
			...
			getThumbnailUrl : function(N) {
				...
				if (R && (R = R[0])) {
					if (Q.config.thumbsEnabled && Q.rQ.rO.test(R)) {
						...
						if (N)
							T.fileHash = S;
						//console.log(U.folder.type);
						// 图片预览路径改成静态资源Url
						return U.folder.getUrl() + P;
						//return domain_image + '/' + U.folder.type + U.folder + P;
	...
	(function() {
		...
		function s(u, v) {
			return '<p>' + u.name + ' (' + k.formatSize(u.size, v, true) + ', '
					+ u.dateF + ', imageURL:'+u.getThumbnailUrl()+')</p>';
		}
		;
		function t(u, v) {
			var w;
	...
	n.toolbar_Full = [ [ 'Upload', 'Refresh', 'Settings' ] ];
	n.toolbar = 'Full';
	(function() {
		o
				.add(
	...
	(function(r, s, t) {
		...
		y.prototype.tF = function(ar, as) {
			...
			as.bT.open('POST', au);
			as.bT.setRequestHeader("X-CSRF-TOKEN", tokenValue);
		};
													...
													.appendHtml(
															A || B || y != 4 ? t
																	: u
																			...
																			+ k
																					.htmlEncode(a.ed)
																			+ "\x3c\057\142\076\074\x2f\x64\x69\x76\x3e");
									}
