$j.extend(loxia.regional['zh-CN'],{
    ...
    "FREIGHT_IS_NULL":"运费不能为空",
    "ORDER_MUST_SET_FREIGHTFEE":"请设置运费"
});
//移除地址
...
//---------------------------------------------------
	        $j(".setFreightFee").click(function(){
	        	 $j("#setFreightFeeDialog").dialogff({type:'open',close:'in',width:'400px',height:'250px'});
	        });
			//取消
			$j("#orderInfoBody").on("click",".setFreightFeeDialogSubmit",function(){
				 var freightFee = $j("#freightFee").val();
				 if(freightFee==null || freightFee=="" ){
					 nps.info(nps.i18n("ORDER_CREATE_TIPS"),nps.i18n("FREIGHT_IS_NULL"));
					 return;
				 }
				 $j("#actualFreight").val(freightFee);
				 freightFeeFlag = true;
				 refreshOrderData();
				 $j("#setFreightFeeDialog").dialogff({type:'close'});
			});
			//取消
			$j("#orderInfoBody").on("click",".setFreightFeeDialogCancel",function(){
				 $j("#setFreightFeeDialog").dialogff({type:'close'});
			});
	        /**
		     * 提交订单
...
//					return;
//				}
			   var json = {"extentionCode": $j(this).attr("extentionCode"),"manualFlag":true};
...
//---------------------------------------------------------------商品列表
		...
		$j("#seachDialog").on("click",".toAddShoppingCart",function(){
			...
			var content = '<div class="ui-block-line">'+
			 ...
			 '<div class="ui-block-line">'+
			   	  '<label>购买价：</label>'+
				  '<div class="pt7">'+
					   '<input type="text" id="buyPrice" onblur="checkNum(this)" placeholder=""/>'+
				  '</div>'+
			 '</div>';
			...
			$j("#addShoppingCart-dialog").dialogff({type:'open',close:'in',width:'500px',height:'360px'});
		});
		...
		$j(".addbuycart").on('click', function(){
			...
			//增加到购物车
			var json = {"skuId": $j('#skuId').val(), "extentionCode": $j('#extentionCode').val(), 
					"quantity":$j("#skuQty").val(),"manualFlag":true,"buyPrice":buyPrice};
...
function orderFormValidate(form){
		...
		if(!freightFeeFlag){
			return nps.info(nps.i18n("ORDER_CREATE_TIPS"),nps.i18n("ORDER_MUST_SET_FREIGHTFEE"));
		}
...
function refreshOrderData(){
	...
	nps.asyncXhr(refreshDataUrl, eval(json), {
		...
		successHandler : function(data) {
			...
			var frontendBaseUrl = data.frontendBaseUrl;
			var customBaseUrl = data.customBaseUrl;
			var smallSize = data.smallSize;
			var pdpPrefix = data.pdpPrefix;
