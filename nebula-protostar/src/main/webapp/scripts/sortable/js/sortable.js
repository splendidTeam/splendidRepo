(function() {
  ...
  sortable = {
    ...
    initTable: function(table) {
      ...
      ths = table.querySelectorAll('.sort-header th');

      var s =(table.querySelectorAll('.sort-header th[data-sorted-direction]').length ==1);
      for (i = _i = 0, _len = ths.length; _i < _len; i = ++_i) {
        ...
        if(s){
            if(th.getAttribute('data-sorted-direction')){
    ...
    setupClickableTH: function(table, th, i) {
      ...
      return addEventListener(th, clickEvent, function(e) {
        ...
        for (_i = 0, _len = ths.length; _i < _len; _i++) {
          var th1 = ths[_i];
          th1.setAttribute('data-sorted', 'false');
          th1.removeAttribute('data-sorted-direction');
        }
        this.setAttribute('data-sorted', 'true');
        this.setAttribute('data-sorted-direction', newSortedDirection);

        if(table.getAttribute('data-sortable-type') == "server"){
            $(table).trigger('sort.sortable',[th , newSortedDirection]);
        }else{
