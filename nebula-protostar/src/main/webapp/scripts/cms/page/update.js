$j.extend(loxia.regional['zh-CN'], {
	...
	"PAGE.UPDATE.AREANAME.DISABALE":"站点path不可用",
	"PAGE.UPDATE.AREANAME.ENALE":"站点path可用",
	"PAGE.UPDATE.SUCCESS":"修改站点成功",
	"PAGE.UPDATE.FAILURE":"修改站点失败",
	"PAGE_AREA_SUCCESS":"页面区域修改成功",
	"PAGE_AREA_FAIL":"页面区域修改失败",
	"PAGE_NOT_UPDATE_PATH":"Path不能被修改",
	"PAGE_NOT_UPDATE_TEMPLATE":"模版不能被修改",
...
//更新page的基本信息
var updateInfoUrl="/cms/updatePage.json";
//动态获取属性
var getPagePropetryUrl="/cms/getPageDynamicPropertyFields.json";
//更新区域
var updatePageAreaUrl="/cms/updatePageContent.json";
//更新属性
var updatePropertyUrl="/cms/updatePageDynamicProperty.json";
//获取模版的区域信息
...
//检验区域或者组件是否存在
var checkElementBlockPathUrl=base+"/cms/checkElementBlockPathExists.json";
//检验模版是否存在
			...
			$j(this).find(".tag-change-ul").find("li").on("click",function(){	
					if($j(this).index()!=0){					
		                ...
		                if($j("#definitionPath").val().trim().length<=0 || $j("#definitionPath").val().trim()==""){
		                	return nps.info(nps.i18n("PAGE_MANAGER_TIPS"),nps.i18n("NO_TEMPLATE"));
		                }
					}	
			});
	});
...
//获取区域信息	
function getAreaElementBlockInfo(){
...
function getPageProepertyInfo(){
	...
	loxia.asyncXhr(getPagePropetryUrl, templatePath,{type: "GET",
		success: function(data){    			
			...
			divNode.html("");
			var proptery=eval(data);//转换json格式    
			var  html="";
			if(proptery.length>0){
    			for(var i=0;i<proptery.length;i++){ 
    				if(proptery[i].required==true){
    					if(proptery[i].type==2 || proptery[i].type==3){
    						html = "<div class='ui-block-line'> <label>"+proptery[i].fieldName+
    								"</label><div > <input type='text' id='"+proptery[i].code+
    								"' name='"+proptery[i].code+"' loxiaType='input'" +
    								" value='' onkeydown='return false;' placeholder='该属性为必填'  mandatory='true'/>";
    					}else{
    						html = "<div class='ui-block-line'> <label>"+proptery[i].fieldName+
    								"</label><div > <input type='text' id='"+proptery[i].code+
    								"' name='"+proptery[i].code+"' loxiaType='"+(proptery[i].editer=='date'?'date':'input')+
    								"' value='' placeholder='该属性为必填'  mandatory='true'/>";
    					}
    				}else{
    					if(proptery[i].type==2 || proptery[i].type==3){
    						 html = "<div class='ui-block-line'> <label>"+proptery[i].fieldName+
    						 		"</label><div > <input type='text' id='"+proptery[i].code+
    						 		"' name='"+proptery[i].code+"' loxiaType='input'"+
    						 		" value='' onkeydown='return false;' placeholder=''  mandatory='false'/>";
    					}else{
    						html = "<div class='ui-block-line'> <label>"+proptery[i].fieldName+
    								"</label><div > <input type='text' id='"+proptery[i].code+
    								"' name='"+proptery[i].code+"' loxiaType='"+(proptery[i].editer=='date'?'date':'input')+
    								"' value='' placeholder=''  mandatory='false'/>";
    					}
    				}
    				if(proptery[i].type==2 || proptery[i].type==3){
	    				html+="<a id='edit_pro' class='func-button ml5' href='javascript:void(0)'> 编辑</a>" +
	    						"<span id='childproperyPath' style='display:none'>"+proptery[i].path+"</span>" +
	    						"<span id='childproperyType' style='display:none'>"+proptery[i].type+"</span>";
	    			}
	    			html+="</div></div>";
    				divNode.append(html);
    			}
	...
	nps.submitForm('pageForm',{mode:'async',
		successHandler : function(data){
			if(data.isSuccess == true){
				//绑定事件
				 var definitionPathValue= $j("#definitionPath").val().trim();
					if(definitionPathValue.length>0 && definitionPathValue!=""){
						bindClick();					
						var cssStr=$j("#definitionPath").attr("class");
						if(cssStr.indexOf("NoEdit")==-1){//只有页面初始化为空，并且选择了模版才获取动态属性和区域信息
							//获取属性    
							getPageProepertyInfo();
						    //根据选择的模版路径读取区域信息			    	
						    getAreaElementBlockInfo();	
						}	
						
						//页面的处理，只读 和隐藏选择按钮
				    	$j(".common-ic.sel-dialog.zoom-ic").remove();
				    	$j("#definitionPath").attr("readonly","readonly");
				    	$j("#definitionPath").attr("class","fLeft NoEdit ui-loxia-default ui-corner-all");
				    	//loxia.initContext($j("#definitionPath"));
				    	$j("#definitionPath").on("click",function(){
				    		 $j("#loxiaTip-r1").show();
				    			 $j("#definitionPathMessge").html(nps.i18n("PAGE_NOT_UPDATE_TEMPLATE"));
				    			 setTimeout(function(){ 
				    				$j("#loxiaTip-r1").hide();
				    			 },2000); 					    		
				    	});
				...
				return nps.info(nps.i18n("PAGE_MANAGER_TIPS"),nps.i18n("PAGE.UPDATE.SUCCESS"));
			}else{
				return nps.info(nps.i18n("PAGE_MANAGER_TIPS"),nps.i18n("PAGE.UPDATE.FAILURE"));
			}	
		}});	
}
...
function checkPath(val,obj){
	...
	//验证路径是否存在
	var r = loxia.syncXhr(checkElementBlockPathUrl,{"elementBlockPath":val},{type: "POST"});
		if(r.isSuccess){
			return nps.i18n("PAGE_AREA_COMPONENT");
		}
		return loxia.SUCCESS;
...
//验证备份节点数
function checkProperty(val, obj){
	if(val=="" || val.length==0){
		return nps.i18n("输入不能为空");;
	}
	return loxia.SUCCESS;
}

//检查一个字符串是否在另外一个集合中
function checkValue(val,arrays){
	var flag=false;
	for(var i=0;i<arrays.length;i++){
		if(val==arrays[i]){
			flag=true;
...
$j(document).ready(function(){
    ...
    //返回按钮
	 ...
	 $j("#editArea").on("click","div.addAreaChildren",function(){
			...
			var html=$j('<div style="margin-bottom:10px;"><input type="text" loxiaType="input" ' +  
	    			'name="parameters"' + 
	    			' style="width:62%;" placeHolder="区域或组件path路径">'+
	    			'<input type="hidden" name="sorts" value="' + sort +  '"/>' +
	    			'<span class="common-ic findArea zoom-ic"></span><span class="common-ic delete-ic new-list-delete"></div>');
	    ...
	    $j("#editArea").on("click","span.findArea",function(){
	    	curSelTreeObj=$j(this);
			 ...
			 tree.getInit();
	    	/*var treeData = loxia.syncXhr(areaAndComponentZNodeUrl,{},{type: "GET"});
			//初始化树
		    $j.fn.zTree.init($j("#areaOrComp"), setting, treeData);*/
	    	$j("#areaOrcomp-dialog").dialogff({type:'open',close:'in',width:'600px',height:'300px'});
	    });
	
	 $j(".sel-dialog").on("click",function(){
		 	//var json = loxia.syncXhr("/cms/queryTemplateZNode.json",{},{type: "GET"});
	    	curSelTreeObj=$j(this);
	 ...
	 $j("#name").on("click",function(){
			 $j("#loxiaTip-r").show();
				 $j("#parentPathMessage").html(nps.i18n("PAGE_NOT_UPDATE_PATH"));
				 setTimeout(function(){ 
					$j("#loxiaTip-r").hide();
				 },2000); 		
		});
