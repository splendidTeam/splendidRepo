...
//保存模板
var saveTemplateUrl = base + "/cms/savePageTemplate.json";
//页面模板
var entryTemplateListUrl = base+'/newcms/pageTemplateList.htm'; 
...
$j(document).ready(function(){
	...
	//模块后退按钮
	$j(".button.return-module").click(function() {
		window.location.href=base+"/newcms/moduleTemplateList.htm";
	});
	
	//关闭html
	$j(".btn-ok").click(function() {
		$j("#dialog-view-html").dialogff({type : 'close'});
	});
    var supportType = $j(".supportType").attr("supportType");
    $j(".supportType option[value='"+supportType+"']").attr("selected",true);
});
