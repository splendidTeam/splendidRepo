$j.extend(loxia.regional['zh-CN'], {
	...
	"PATH.ISNOTNULL":"path不能为空",
	"PATH.CANNOT.UPDATE":"path不可修改，是否继续保存？",
	"DIALOG.DELETE.COMFIRM":"确认删除？",
	"DIALOG.ADD.SUCCESS":"属性基本信息保存成功",
	"DIALOG.ADD.FAILURE":"属性基本信息保存失败",
...
$j(document).ready(function() {
   ...
   */
	$j(".button.orange.tempselok").on("click", function() {
		...
		if (nodes_.length == 0) {
			nps.info(nps.i18n("DIALOG.MANAGER.TIPS"),nps.i18n("DIALOG.PLEASE.SELECT.EXTEND"));
			return false;
		} else {
			// 取到选择的行业名称
...
//			inputObj.blur();
	...
	$j(".button.orange.save").eq(0).on("click", function() {
		if($j("#path").val().trim()==""){
			$j("#path").addClass("ui-loxia-error");
			return nps.info(nps.i18n("DIALOG.MANAGER.TIPS"),nps.i18n("PATH.ISNOTNULL"));
		}
		if(!checkPathExists()){
			$j("#path").addClass("ui-loxia-error");
	    	return nps.info(nps.i18n("DIALOG.MANAGER.TIPS"),nps.i18n("PATH.DISABALE"));
	    }
		nps.confirm(nps.i18n("DIALOG.MANAGER.TIPS"),nps.i18n("PATH.CANNOT.UPDATE"), function(){
