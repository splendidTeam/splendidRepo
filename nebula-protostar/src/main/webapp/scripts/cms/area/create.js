$j.extend(loxia.regional['zh-CN'], {
	...
	"AREA.ADD.SUCCESS":"添加区域成功",
	"AREA.ADD.FAILURE":"添加区域失败",
	...
	"NO_SELECT_TEMPLATE":"保存基本信息时未选择模版路径，不能编辑其它信息",
...
function userFormValidate(form) {
	...
	if(definitionPath.length > 0){
		if((definitionPath == '/' && definitionPath.length == 1) ||  definitionPath.indexOf("/")!=0 ||
				(definitionPath.length > 1 && definitionPath.substring(definitionPath.length-1,definitionPath.length)=="/")){
			flag = true;
			return nps.i18n("TEMPLATE_PATH_DISABLE");
		}
		var json = {"definitionPath" : (definitonPathPrefix + definitionPath)};
		var _d = loxia.syncXhr(validateDefinitionPathUrl,json,{type: "GET"});
		if(!_d.isSuccess){
			result += nps.i18n("TEMPLATE_PATH_DISABLE");
		}
	}
...
$j(document).ready(function() {
    ...
    $j(".button.preview").click(function(){
    	...
    	if(!isSave){
    		return nps.info(nps.i18n("AREA_MANAGER_TIPS"),nps.i18n("AREA.ADD.MSG"));
    	}else if(!checkStrIsNotNull(definitionPath)){
    		return nps.info(nps.i18n("AREA_MANAGER_TIPS"),nps.i18n("NO_SELECT_TEMPLATE"));
    	}
		...
		.ready(
				function() {
					...
					$j(".button.orange.save").eq(0).on("click", function() {
						var name = $j.trim($j("#name").attr("value"));
						var elementName = $j.trim($j("#elementName").attr("value"));
						var definitionPath = $j.trim($j("#definitionPath").attr("value"));
						if(name.length == 0 || elementName.length == 0){
						...
						if(!checkStrIsNotNull(definitionPath)){
							 nps.submitForm('userForm',{mode: 'async', 
									...
									if(data.isSuccess == true){
										...
										isClick = true;
										isSave=true;	
										return nps.info(nps.i18n("AREA_MANAGER_TIPS"),nps.i18n("AREA.ADD.SUCCESS"));
									}else
									{
										return nps.info(nps.i18n("AREA_MANAGER_TIPS"),nps.i18n("AREA.ADD.FAILURE"));
									}
							}});
						}else{
								 ...
								 nps.submitForm('userForm',{mode: 'async', 
										...
										if(data.isSuccess == true)
										{
											...
											for(var i=0 ;i < elementBlocks.length;i++){
												...
												var start = '<div class="ui-block-content border-grey" style="margin-bottom:10px;"> <div class="ui-block-line "> <input type="hidden" name="lockeds" value="'
													        + locked
													        + '"/> <label class="">';
					 ...
					 $j("#editArea").on("click","div.addAreaChildren",function(){
							...
							var html=$j('<div style="margin-bottom:10px;">'
					    			...
					    			+  'name="parameters"'  
					    			+' style="width:62%;" checkmaster="validatePath" placeHolder="区域或组件path路径">'
					    			+ '<input type="hidden" name="sorts" value="'
					    			+ sort 
					    			+  '"/>' 
