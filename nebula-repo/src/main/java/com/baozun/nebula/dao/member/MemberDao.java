...
public interface MemberDao extends GenericEntityDao<Member,Long>{
	 ...
	 */
	@NativeQuery(model = Member.class)
	List<Member> findMemberEffectiveMemberIdByGroupId( 
	...
	/** 查询第三方会员*/
	@NativeQuery(model = Member.class)
	Member findThirdMemberByUidAndSource(@QueryParam("thirdPartyIdentify")String thirdPartyIdentify,@QueryParam("source")Integer source,
	...
	@NativeQuery(alias = "id", clazzes = Long.class)
	List<Long> findMemberIdsByComsumeeAmtPromotionId(@QueryParam("shopId") Long shopId,
			@QueryParam("amt") BigDecimal amt,@QueryParam("prmId") String prmId);
}
