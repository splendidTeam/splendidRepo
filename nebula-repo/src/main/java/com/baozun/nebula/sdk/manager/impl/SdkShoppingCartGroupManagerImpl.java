...
public class SdkShoppingCartGroupManagerImpl implements SdkShoppingCartGroupManager {
	...
	public List<ShoppingCartLineCommand> groupShoppingCartLinesToDisplayByLinePromotionResult(
			ShoppingCartCommand oneShoppingCart, List<PromotionBrief> promotionBriefList) {
		...
		for (PromotionCommand onePromotion : allLinePromotionList) {
			...
			onePromotionSettingSKUList = getOnePromotionSKUSettingListByPromotionBriefs(promotionBriefList,
					onePromotion.getPromotionId());
			onePromotionConditionSKUList = getPromotionConditionSKUListFromShoppingCartLines(
					oneShoppingCart.getShoppingCartLineCommands(), onePromotion.getPromotionId());
			if (this.checkPromotionHasSuitKits(onePromotion)) {
				suitLines = new ArrayList<ShoppingCartLineCommand>();
				// 套餐行
				suitLines = this.getLinesOfSuitKitsPromotion(oneShoppingCart.getShoppingCartLineCommands(),
						onePromotionSettingSKUList, onePromotionConditionSKUList);
				...
				// 买赠行整合
				lineGiftLines = this.getLinesOfLineGiftPromotion(oneShoppingCart.getShoppingCartLineCommands(),
						onePromotionSettingSKUList, onePromotionConditionSKUList);
				...
				// 整单赠品
				orderGiftLines = this.getLinesOfOrderGiftPromotion(oneShoppingCart.getShoppingCartLineCommands(),
				...
				normalAMTLines = this.getLinesOfNormalPromotion(oneShoppingCart.getShoppingCartLineCommands(),
						onePromotionSettingSKUList, onePromotionConditionSKUList);
		...
		allGroupedLines = appendShoppingCartLinesOfNoPromotion(oneShoppingCart.getShoppingCartLineCommands(),
				allGroupedLines);
		// 设置选中赠品行
		allGroupedLines = setGiftLinesSettlement(allGroupedLines, memberId);
		// 设置行库存状态
		if ((allGroupedLines != null && allGroupedLines.size() > 0)
				&& (allLinePromotionList != null && allLinePromotionList.size() > 0))
	...
	private List<ShoppingCartLineCommand> setShoppingCartLineOutOfStockBySKUId(
			List<ShoppingCartLineCommand> groupedLines, List<PromotionCommand> linePromotions) {
...
//						line.setValid(false);
		...
		for (ShoppingCartLineCommand line : allGroupedLines) {
			if (line.isCaptionLine()) {
				...
				if (line.getLineGroup() == null || line.getLineGroup() == 0L)
					log.info("店铺编号：" + line.getShopId() + ",活动编号：" + line.getPromotionIds() + ",常   规   行：-------------ID：" + line.getId()
							+ ",ItemID：" + line.getItemId() + ",SKU ID：" + line.getSkuId() + ",ExtCode："
							+ line.getExtentionCode() + ",名        称：" + line.getItemName() + ",价        格："
							+ line.getSalePrice() + ",优        惠：" + line.getDiscount() + ",件数：" + line.getQuantity()
							+ ",库存：" + line.getStock());
				else {
					lineGift = "店铺编号：" + line.getShopId() + ",分组编号：" + line.getLineGroup() + ",活动编号：" + line.getPromotionIds() + ",----赠品："
							+ (line.isGift() == true ? "是，" : "否，") + "套装：" + (line.isSuitLine() == true ? "是，" : "否，")
							+ "ID：" + line.getId() + ",ItemID：" + line.getItemId() + ",SKU ID：" + line.getSkuId()
							+ ",ExtCode：" + line.getExtentionCode() + ",名        称：" + line.getItemName()
							+ ",价        格：" + line.getSalePrice() + ",优        惠：" + line.getDiscount() + ",件数："
							+ line.getQuantity() + ",库存：" + line.getStock() + ",选中：" + line.getSettlementState();
					if (line.isGift() == true) {
	...
	private List<ShoppingCartLineCommand> getLeftLinesAfterRemoveOneGroup(List<ShoppingCartLineCommand> containsLines,
		...
		for (ShoppingCartLineCommand containLine : containsLines) {
			...
			for (ShoppingCartLineCommand removeLine : removeLines) {
				if (containLine.getLineGroup().equals(removeLine.getLineGroup())
						&& containLine.getSkuId().equals(removeLine.getSkuId())) {
					exists = true;
					break;
				}
			}
	 ...
	 */
	@Transactional(readOnly=true)
	private List<PromotionCommand> appendPromotionsByPromotionIdList(List<PromotionCommand> tmpList,
	...
	public List<ShoppingCartLineCommand> getShoppingCartLinesByPromotions(
			List<ShoppingCartLineCommand> linesNoCaptions, List<Long> promotionList) {
	 ...
	 * @throws
	 */
	private List<ShoppingCartLineCommand> getShoppingCartLineByGroup(
			List<ShoppingCartLineCommand> shoppingCartLineCommands, Long lineGroup) {
		...
		for (ShoppingCartLineCommand line : lines) {
			if (line.isGift() && GiftChoiceType.NeedChoice.equals(line.getGiftChoiceType())) {
				...
				for (ShoppingCartLineCommand gift : giftLinesSeleted) {
					...
					// continue;
					if (line.getPromotionId().equals(gift.getPromotionId())
							&& line.getItemId().equals(gift.getItemId())
						...
						if (gift.getSettlementState() != null
								&& Constants.CHECKED_CHOOSE_STATE.equals(gift.getSettlementState()))
	...
	public List<PromotionSKUDiscAMTBySetting> getOnePromotionSKUSettingListByPromotionBriefs(
			List<PromotionBrief> promotionBriefList, Long onePromotionId) {
	...
	public List<PromotionConditionSKU> getPromotionConditionSKUListFromShoppingCartLines(
			List<ShoppingCartLineCommand> shoppingLines, Long promotionId) {
	 ...
	 */
	@Override
	public List<ShoppingCartLineCommand> getLinesOfSuitKitsPromotion(List<ShoppingCartLineCommand> oneShopCartLineList,
	...
	public List<ShoppingCartLineCommand> getLinesOfPromotionBySettingAndConditionList(
		...
		if (promotionConditionSKUList != null && promotionConditionSKUList.size() > 0) {
			for (PromotionConditionSKU conditionSKU : promotionConditionSKUList) {
				if (onePromotion == null)
					onePromotion = this.sdkPromotionRuleFilterManager.getPromotionByPromotionId(conditionSKU
							.getPromotionId());
		...
		if (promotionSettingSKUList != null && promotionSettingSKUList.size() > 0) {
			for (PromotionSKUDiscAMTBySetting settingSKU : promotionSettingSKUList) {
				if (onePromotion == null)
					onePromotion = this.sdkPromotionRuleFilterManager.getPromotionByPromotionId(settingSKU
							.getPromotionId());
				...
				if (settingSKU.getGiftMark() == false) {
					oneTempLine = this
							.getShoppingCartLineBySKUId(oneShopCartLineList, lineGroup, settingSKU.getSkuId());
					if (null == oneTempLine)
						continue;
					if (!this.checkLineExistsByLineGroupAndSKU(allPromotionLines, lineGroup, oneTempLine.getSkuId())) {
						// 设置优惠金额
					...
					oneTempLine = this.getShoppingCartGiftLineByItemId(oneShopCartLineList, lineGroup,
							settingSKU.getItemId());
					if (null == oneTempLine)
						continue;
					oneTempLine.setLineGroup(lineGroup);
					allPromotionLines.add(oneTempLine);
	 ...
	 * @return
	 */
	public Boolean checkLineExistsByLineGroupAndSKU(List<ShoppingCartLineCommand> shopCartLineList, Long lineGroup,
			Long skuId) {
	 ...
	 * @return
	 */
	public ShoppingCartLineCommand getShoppingCartLineBySKUId(List<ShoppingCartLineCommand> shopCartLineList,
			Long lineGroup, Long skuId) {
	...
	public ShoppingCartLineCommand getShoppingCartGiftLineByItemId(List<ShoppingCartLineCommand> shopCartLineList,
			Long lineGroup, Long itemId) {
		ShoppingCartLineCommand lineTmp = new ShoppingCartLineCommand();
		for (ShoppingCartLineCommand line : shopCartLineList) {
			if (line.isCaptionLine())
				continue;
	 ...
	 * @return
	 */
	public Boolean checkPromotionHasLineGift(List<PromotionSKUDiscAMTBySetting> onePromotionSettingSKUList,
			PromotionCommand onePromotion) {
	 ...
	 */
	@Override
	public List<ShoppingCartLineCommand> getLinesOfLineGiftPromotion(List<ShoppingCartLineCommand> oneShopCartLineList,
		...
		List<ShoppingCartLineCommand> lines = getLinesOfPromotionBySettingAndConditionList(oneShopCartLineList,
				lineGiftPromotionSettingSKUList, lineGiftPromotionConditionSKUList);
		for (ShoppingCartLineCommand line : lines) {
	 ...
	 */
	@Override
	public Boolean checkPromotionOrderGift(List<PromotionSKUDiscAMTBySetting> onePromotionSettingSKUList,
			PromotionCommand onePromotion) {
		Boolean orderBase = checkOrderBaseByPromotion(onePromotion);
		Boolean hasGift = false;
		if (orderBase == false)
	 ...
	 * @return
	 */
	public List<ShoppingCartLineCommand> appendShoppingCartLines(List<ShoppingCartLineCommand> linesBefore,
		...
		for (ShoppingCartLineCommand curLine : currentPromotionLines) {
			...
			for (ShoppingCartLineCommand befLine : linesBefore) {
				...
				if (befLine.getId() != null && curLine.getId() != null && befLine.getId().equals(curLine.getId())) {
					existInBefore = true;
					break;
				} else if (befLine.getLineGroup().equals(curLine.getLineGroup())
						&& befLine.getSkuId().equals(curLine.getSkuId())) {
					existInBefore = true;
					break;
				}
			}
	 ...
	 */
	@Override
	public List<ShoppingCartLineCommand> getLinesOfNormalPromotion(List<ShoppingCartLineCommand> oneShopCartLineList,
			List<PromotionSKUDiscAMTBySetting> onePromotionSettingSKUList,
		...
		linesNeedToCompress = getLinesOfPromotionBySettingAndConditionList(oneShopCartLineList,
				onePromotionSettingSKUList, onePromotionConditionSKUList);
	...
	@Transactional(readOnly=true)
	public List<PromotionConditionSKU> getPromotionConditionSKUListFromShoppingCartLines(
			List<ShoppingCartLineCommand> shoppingLines, List<Long> promotionIdList) {
		if (shoppingLines == null || shoppingLines.size() == 0 || promotionIdList == null
				|| promotionIdList.size() == 0)
			return null;
		List<PromotionConditionSKU> suitSKUList = new ArrayList<PromotionConditionSKU>();
	 ...
	 * 按获取条件范围内的商品行
	 */
	private List<PromotionConditionSKU> getAllSuitSKUList(List<ShoppingCartLineCommand> shoppingLines,
	 ...
	 * 按Item获取条件范围内的商品行
	 */
	private List<PromotionConditionSKU> getProductSuitSKUList(List<ShoppingCartLineCommand> shoppingLines, Long itemId,
			AtomicCondition condition) {
		List<PromotionConditionSKU> skuList = new ArrayList<PromotionConditionSKU>();
		PromotionConditionSKU sku = null;
		if (null != shoppingLines && shoppingLines.size() > 0) {
	 ...
	 */
	private List<PromotionConditionSKU> getCategorySuitSKUList(List<ShoppingCartLineCommand> shoppingLines,
			Long categoryId, AtomicCondition condition) {
		List<PromotionConditionSKU> skuList = new ArrayList<PromotionConditionSKU>();
		PromotionConditionSKU sku = null;
		if (null != shoppingLines && shoppingLines.size() > 0) {
			for (ShoppingCartLineCommand shoppingLine : shoppingLines) {
	 ...
	 * 按Combo获取条件范围内的商品行
	 */
	private List<PromotionConditionSKU> getComboSuitSKUList(List<ShoppingCartLineCommand> shoppingLines, Long comboId,
			AtomicCondition condition) {
		List<PromotionConditionSKU> skuList = new ArrayList<PromotionConditionSKU>();
		PromotionConditionSKU sku = null;
		if (null != shoppingLines && shoppingLines.size() > 0) {
	 ...
	 */
	@Override
	public List<PromotionConditionSKU> getConditionSKUListByPromotionId(List<PromotionConditionSKU> listSKUCondition,
			Long promotionId) {
	...
	public List<PromotionSKUDiscAMTBySetting> getSettingSKUListByPromotionId(
			List<PromotionSKUDiscAMTBySetting> skuListSetting, Long promotionId) {
	 ...
	 * 按Item获取条件范围内的商品行
	 */
	private List<PromotionConditionSKU> getProductConditionSKUList(List<ShoppingCartLineCommand> shoppingLines,
			Long itemId, AtomicCondition condition) {
		List<PromotionConditionSKU> skuList = new ArrayList<PromotionConditionSKU>();
		PromotionConditionSKU sku = null;
		if (null != shoppingLines && shoppingLines.size() > 0) {
			for (ShoppingCartLineCommand shoppingLine : shoppingLines) {
	 ...
	 * 按Category获取条件范围内的商品行
	 */
	private List<PromotionConditionSKU> getCategoryConditionSKUList(List<ShoppingCartLineCommand> shoppingLines,
			Long categoryId, AtomicCondition condition) {
		List<PromotionConditionSKU> skuList = new ArrayList<PromotionConditionSKU>();
		PromotionConditionSKU sku = null;
		if (null != shoppingLines && shoppingLines.size() > 0) {
	...
	private List<PromotionConditionSKU> getCustomScopeConditionSKUList(List<ShoppingCartLineCommand> shoppingLines,
			Long customId, AtomicCondition condition) {
		List<PromotionConditionSKU> skuList = new ArrayList<PromotionConditionSKU>();
		PromotionConditionSKU sku = null;
	...
	private List<PromotionConditionSKU> getCustomConditionSKUList(List<ShoppingCartLineCommand> shoppingLines,
			Long customId, AtomicCondition condition) {
		List<PromotionConditionSKU> skuList = new ArrayList<PromotionConditionSKU>();
		PromotionConditionSKU sku = null;
		Long promotionId = 0L;
	 ...
	 * 按Combo获取条件范围内的商品行
	 */
	private List<PromotionConditionSKU> getComboConditionSKUList(List<ShoppingCartLineCommand> shoppingLines,
			Long comboId, AtomicCondition condition) {
		List<PromotionConditionSKU> skuList = new ArrayList<PromotionConditionSKU>();
		PromotionConditionSKU sku = null;
		if (null != shoppingLines && shoppingLines.size() > 0) {
			for (ShoppingCartLineCommand shoppingLine : shoppingLines) {
	 ...
	 * 按获取条件范围内的商品行
	 */
	private List<PromotionConditionSKU> getAllConditionSKUList(List<ShoppingCartLineCommand> shoppingLines,
			AtomicCondition condition) {
		List<PromotionConditionSKU> skuList = new ArrayList<PromotionConditionSKU>();
		PromotionConditionSKU sku = null;
		if (null != shoppingLines && shoppingLines.size() > 0) {
