		...
		for (PromotionCommand outCmd : promotionList) {
			...
			for (PromotionCommand inCmd : promotionList) {
				if (!outCmd.equals(inCmd)) {
					...
					if (sdkFilterManager.isMemberConfilct(inMemberExp, outMemberExp) && sdkFilterManager.isProductConfilct(inProductExp, outProductExp, shopId)) {
						result.add(outCmd);
						break;
					}
				}
...
//		}
	...
	private int generatePriorityStatus(Integer activeMark, Date endTime) {
		if ((PromotionPriorityAdjust.ACTIVEMARK_ENABLE.equals(activeMark) || PromotionPriorityAdjust.ACTIVEMARK_DISABLE.equals(activeMark)) && endTime.compareTo(new Date()) < 0) {
	...
	public Long saveOrUpdatePriority(PromotionPriorityAdjustCommand cmd) {
		...
		List<PromotionPriorityAdjustDetail> detailList = (List<PromotionPriorityAdjustDetail>) JSONArray.toCollection(json, PromotionPriorityAdjustDetail.class);
		...
		if (null == cmd.getId()) { // 新建
			...
			for (PromotionPriorityAdjustDetailCommand d : promotionPriorityAdjustDetailDao.findPromotionPriorityAdjustDetailByAdjustid(priority.getId())) {
				idList.add(d.getId());
			}
...
//		}
	...
	private void logPromotionAdjustPriority(List<PromotionPriorityAdjustDetailCommand> allAdjustList)
	{
		...
		{
			log.info( "活动编号："+one.getPromotionId() + "，优先级：" + one.getPriority() + "，活动分组："+one.getGroupName() + "，拍他标签：" + one.getExclusiveMark());
		}
		log.info("结束！");
	}
	private void logPromotionPriority(List<PromotionCommand> allPromotionList)
	{
		...
		for(PromotionCommand one:allPromotionList)
		{
			...
			log.info( "活动编号："+one.getPromotionId() + "，优先级：" + one.getPriority() + "，缺省排他：" + one.getExclusiveMark()+ "，排他组类型：" + exclusiveType + "，拍他组名：" + one.getGroupName() + "，确省优先级：" + one.getDefaultPriority() + "，活动名称："+one.getPromotionName());
		}
		log.info("结束！");
	}
	/**
	 ...
	 * @return
	 */
	private List<PromotionCommand> promotionAdjustmentByAdjustPriority(List<PromotionCommand> originalPromosList, List<PromotionPriorityAdjustDetailCommand> promotionProAdjustList) {
	...
	@Override
	@Transactional
	public Integer enableOrUnablePriorityById(Long id, Long userId,
			Integer activeMark) {
	...
	@Override
	@Transactional(readOnly=true)
	public List<PromotionPriorityAdjustCommand> findEffectivePriorityList(
			Long shopId) {
