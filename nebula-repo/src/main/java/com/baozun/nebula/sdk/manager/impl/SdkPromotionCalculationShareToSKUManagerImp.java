...
public class SdkPromotionCalculationShareToSKUManagerImp implements SdkPromotionCalculationShareToSKUManager {
	...
	public BigDecimal getLineDiscAMTShareOnOrderBaseByShopIdAndSKUId(List<PromotionSKUDiscAMTBySetting> shareList, Long shopId, Long skuId) {
		...
		for (PromotionSKUDiscAMTBySetting skuSet : shareList) {
			if (skuSet.getBaseOrder() == true && skuSet.getShopId().longValue() == shopId.longValue() && skuSet.getSkuId().longValue() == skuId.longValue()) {
		...
		for (Entry<Long, List<PromotionBrief>> entry : shopIdSetBriefList.entrySet()) {
			...
			for (PromotionBrief onePromotion : briefList) {
				...
				for (PromotionSettingDetail detail : onePromotion.getDetails()) {
					if (detail.getFreeShippingMark() || detail.getAffectSKUDiscountAMTList() == null || detail.getAffectSKUDiscountAMTList().size() == 0)
						continue;
				...
				if (detailListOrderBase != null && detailListOrderBase.size() > 0) {
					onePromotionShareBySKUOrderBase = sharePromotionDiscountToEachLineByShop(shopCartOne, detailListOrderBase, allPromotionShareBySKULineBase);
				}
	...
	private List<PromotionSKUDiscAMTBySetting> convertLinePromotionSetting(ShoppingCartCommand shopCart, List<PromotionSettingDetail> settingList) {
	...
	private BigDecimal getDiscAMTFromPromotionSKUDiscAMTBySettingsBySKUId(List<PromotionSettingDetail> skuBaseDiscAMTSettingList, long skuId) {
	...
	private List<PromotionSKUDiscAMTBySetting> sharePromotionDiscountToEachLineByShop(ShoppingCartCommand shopCart, List<PromotionSettingDetail> orderBaseSettingList, List<PromotionSettingDetail> skuBaseDiscAMTSettingList) {
