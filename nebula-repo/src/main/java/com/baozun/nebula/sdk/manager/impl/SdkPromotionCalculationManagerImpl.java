...
public class SdkPromotionCalculationManagerImpl implements SdkPromotionCalculationManager {
	...
	public List<PromotionBrief> calculationPromotion(ShoppingCartCommand shopCart, List<PromotionCommand> allPromotionList) {
		log.info("促销引擎开始！");
		if (shopCart == null) {
			return null;
		}

		Map<Long, ShoppingCartCommand> shopCartAll = shopCart.getShoppingCartByShopIdMap();// 按店铺取购物车
		...
		Map<Long, List<PromotionCouponCodeCommand>> couponsAllByShopId = sdkPromotionSettingManager.seperateCouponCodesByShopID(shopCart.getCoupons());
		...
		//try {
			...
			for (Long oneShopId : shopIdSet) {
				...
				if (oneShopPromotionList == null || oneShopPromotionList.size() == 0) {
					...
					/*
					List<ShoppingCartLineCommand> oneSuitLines = new ArrayList<ShoppingCartLineCommand>();
					
					allSuitPromotionList = getSuitPromotionsFromAllofOneShop(oneShopPromotionList);
					for (PromotionCommand suitPromotion : allSuitPromotionList) {
						oneSuitLines = getSuitLinesFromPromotion(shopCart.getShoppingCartLineCommands(),suitPromotion);
						
						PromotionBrief briefListOne = calculationPromotionSuit(oneSuitLines,suitPromotion,shopCartOne.getUserDetails(),briefListAll);
						if (briefListOne != null)
							briefListAll.add(briefListOne);
						//TODO remove the suit lines from normal shopping cart
					}*/
		...
		//} catch (Exception e) {
		//	log.error("促销引擎异常结束！请查看前一个促销！" + e.getMessage());
		//	e.printStackTrace();
		//}
		log.info("促销引擎正常结束！");
		log.info("订单金额：" + shopCart.getOriginPayAmount());
	 ...
	 * 一个优惠设置
	 * 
	 * @see com.baozun.nebula.sdk.manager.SdkPromotionCalculationManager#calculationPromotionByAtomicSetting(com.baozun.nebula.sdk.command.shoppingcart.ShoppingCartCommand,
	...
	public PromotionSettingDetail calculationPromotionByAtomicSetting(ShoppingCartCommand shopCart, PromotionCommand promotion, AtomicSetting setting, List<PromotionBrief> briefListPrevious) {
		...
		if (setting.getSettingTag().equalsIgnoreCase(SettingType.EXP_SCPORDDISC) || setting.getSettingTag().equalsIgnoreCase(SettingType.EXP_SCPORDRATE) || setting.getSettingTag().equalsIgnoreCase(SettingType.EXP_SCPPCSDISC)
			...
			// 当前范围按SKU下ItemID存放
			if (flagExistSinglePrdInCondition
					&& (setting.getSettingTag().equalsIgnoreCase(SettingType.EXP_SCPPRDDISC) || setting.getSettingTag().equalsIgnoreCase(SettingType.EXP_SCPPRDRATE) || setting.getSettingTag().equalsIgnoreCase(SettingType.EXP_SCPPCSDISC) || setting
							.getSettingTag().equalsIgnoreCase(SettingType.EXP_SCPPCSRATE))) {
	 ...
	 * 
	 * @see com.baozun.nebula.sdk.manager.SdkPromotionCalculationManager#getPromotionCalculationBriefDetailByAtomicSetting(com.baozun.nebula.sdk.command.shoppingcart.ShoppingCartCommand,
	 *      com.baozun.nebula.command.promotion.PromotionCommand,
	 *      com.baozun.nebula.calculateEngine.condition.AtomicSetting)
	 */
	@Override
			...
			if (setting.getSettingTag().equalsIgnoreCase(SettingType.EXP_SCPCOUPON)) {
				...
				if (filteredCouponsByTypeId != null && filteredCouponsByTypeId.size() > 0) {
					...
					if (couponType2Check.intValue() == PromotionCoupon.TYPE_RATE.intValue() && setting.getMultiplicationFactor() > MULTIPONE && setting.getOnePieceMark()) {
						...
						discList = sdkPromotionSettingManager.getDiscountAMTByCustomItemIdsCoupon(shopCart.getShoppingCartLineCommands(), customItemIds, couponTypeId, filteredCouponsByTypeId, setting.getOnePieceMark(), promotion.getShopId(), briefListPrevious);
					}
				}
			}
			if (discList != null && discList.size() > 0) {
	...
	private void logBriefsByPromotion(List<PromotionBrief> briefListOnePromotion) {
		...
		for (PromotionBrief brief : briefListOnePromotion) {
			log.info("活动编号：" + brief.getPromotionId().toString() + "，名称:" + brief.getPromotionName() + ",店铺编号：" + brief.getShopId().toString());
			...
			for (PromotionSettingDetail detail : brief.getDetails()) {
				log.info("---设置类型:" + detail.getSettingTypeTag() + ",设置表达式：" + detail.getSettingExpression() + "，优惠金额：" + (detail.getDiscountAmount() == null ? "0" : detail.getDiscountAmount()));
				skulist = detail.getAffectSKUDiscountAMTList();
				if (skulist != null && skulist.size() > 0) {
					for (PromotionSKUDiscAMTBySetting setting : skulist) {
						log.info("------PID:" + setting.getItemId() + "------SKU:" + setting.getSkuId() + ",店铺编号：" + setting.getShopId().toString() + ",名称：" + setting.getItemName() + ",QTY：" + setting.getQty() + ",单价：" + setting.getSalesPrice() + "，优惠金额："
