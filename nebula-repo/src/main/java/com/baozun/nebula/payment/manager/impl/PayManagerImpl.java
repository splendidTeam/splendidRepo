...
public class PayManagerImpl implements PayManager {
	...
	public static final Logger log = LoggerFactory
			.getLogger(PayManagerImpl.class);
	 ...
	 * @param returnVal
	 */
	private void savePayLog(Date createTime, String message, String operator,
			String returnVal) {
		sdkPaymentManager.savePaymentLog(createTime, message, operator, returnVal);
	}

	@Override
	public void updatePayInfos(PaymentResult paymentResult,
			String operator,Integer payType,boolean flag,HttpServletRequest request) {
		if(flag){
...
//				set THIRD_PAY_NO = :thirdPayNo,MODIFY_TIME= :modifyTime,THIRD_PAY_ACCOUNT = :thirdPayAccount,PAY_SUCCESS_STATUS = :paySuccessStatus
				...
				if(salesOrderCommand.getFinancialStatus() != SalesOrder.SALES_ORDER_FISTATUS_FULL_PAYMENT){
					...
					sdkOrderService.updateOrderFinancialStatus(salesOrderCommand.getCode(),
	 ...
	 * @see com.baozun.nebula.payment.manager.PayManager#closeTradeUpdateOrderStatusNew(java.lang.Long, com.baozun.nebula.utilities.integration.payment.PaymentResult, java.lang.String, java.lang.String, java.lang.String, java.lang.String, java.lang.Boolean)
	 */
	@Override
	public void closeTradeUpdateOrderStatus(Long orderId,
			PaymentResult paymentResult, String operator, String orderNo,
			String userType, String payCode, Boolean isOms) {
		/** 扩展点 修改外部code **/
		if( null != salesOrderHandler){
			salesOrderHandler.updateOutCoupon(orderNo, PromotionCouponCode.NOT_USED);
		}
	...
	@Override
	public void savePaymentRequestPaymentLog(PaymentRequest paymentRequest,
			String operator) {
		savePayLog(new Date(),Constants.PAY_LOG_CALL_AFTER_MESSAGE, operator,MapConvertUtils.transPaymentRequestToString(paymentRequest));
	}
	...
	@Override
	public void cancelOrderUpdateInfo(Long orderId, String code,
			Integer logisticsStatus, Boolean isOms) {
		/** 扩展点 修改外部code**/
		if( null != salesOrderHandler){
			salesOrderHandler.updateOutCoupon(code, PromotionCouponCode.NOT_USED);
		}
	...
	public void cancelOrder(BeforePaymentCancelOrderCommand beforePaymentCancelOrderCommand) {
		...
		if(salesOrderCommand.getLogisticsStatus().equals(SalesOrder.SALES_ORDER_STATUS_CANCELED)
				||salesOrderCommand.getLogisticsStatus().equals(SalesOrder.SALES_ORDER_STATUS_SYS_CANCELED)
				){
		...
		if(null == payInfoLog || (payInfoLog.getCallCloseStatus()!=null && payInfoLog.getCallCloseStatus())
				...
				if(Validator.isNotNullOrEmpty(paymenInfo)&& Boolean.parseBoolean(paymenInfo)){
					...
					if(payType.toString().equals(PAYTYPE_WXPAY) || payType.toString().equals(PAYTYPE_UNIONPAY)){
						...
						}else if(ORDERNOTEXIST.equals(res.getMessage())
								||REVOKED.equals(res.getMessage())){
	...
	private boolean tradeOtherStatusHandler(
			BeforePaymentCancelOrderCommand beforePaymentCancelOrderCommand,
	...
	private boolean tradeSuccessHandler(String orderCode,
			SalesOrderCommand salesOrderCommand, boolean isPaySuccess,
			PaymentResult res) {
		String result ="";
...
//		                         updatePayInfos(paymentServiceReturnCommand, null );
		...
		// log
		PayWarnningEvent payWarnningEvent = new PayWarnningEvent(this,orderCode, 
				res.getPaymentStatusInformation().getTradeNo(), new Date(),
				salesOrderCommand.getFinancialStatus().toString(), res.getPaymentStatusInformation().getTradeStatus(), true, result); 
