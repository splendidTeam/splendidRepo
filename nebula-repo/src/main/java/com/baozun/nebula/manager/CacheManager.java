...
/**
 * cahce接口,在底层封装了jedis的api 业务方法的缓存 1.缓存读取步骤： *先通过key以及field查找缓存
 * *如果查不到，则查询数据源(db,solr,file等) ；如果查到，直接返回缓存数据 *将数据源的数据保存到缓存中，并返回 2.无参数方法的缓存：
 * 直接使用key/value来保存缓存 3.带参数方法的缓存： *使用redis的map结构来保存缓存数据(这边有专门的方法如setMapObject)
 * *使用key定位方法
 * *field为参数的组合，在方法内能唯一指向数据，根据需要自己将参数拼装为field,如简单类型：p1-v1-p2-v2;如列表：v1-v2-v3;对象f1-f2-f3
 * *无参数时，field为默认值default-field; 4.缓存方法的嵌套： *原则上允许嵌套使用，但不建议嵌套及过多的层级嵌套
 * *最内层的缓存方法表示为方法体内不会有其它读取缓存的方法，方法名称没有专门的标识 *外层的缓存方法需要设置过期时间,默认为一小时 5.缓存的更新：
 ...
 * *如：某方法参数是多个对象拼成的，但我只是更新了某个对象的数据，完全不清楚其它对象应该传哪些参数 6.缓存的删除:
 ...
 * 7.哪些数据可以被缓存： *原则上只读的业务方法都可以加上缓存 *推荐是访问量大，但修改少的数据 *绝对不允许将写的次数多于读的次数的数据加入缓存
 * 8.方法命名规则 *读取缓存的方法必须以fromCache结尾 *更新缓存的方法以refreshCache结尾
 * *删除缓存的方法以removeCache结尾 *它们是成对的存在，除了后辍，应该一致(有时候不一定会有更新缓存的方法) 9.key的定义原则
 * *key必须定义到CacheKeyConstant中,不允许写死在方法中
 * *key的规则为MC_[package]_[className]_[methodName] *MC表示方法缓存 *package为全包名
 * *classname为接口的类名 *methodName为方法名 临时存储的应用(待收集较齐全后再来完善)
 * 1.限制用户单位时间内的操作次数，如登录密码错十次，淘宝就暂时不让此人登录了 2.暂时存储一些统计数据，如点击量，高并发时参与活动等 3.最新上架商品
 * 不同数据结构的应用场景： 1.key/value：将查询出的单个对象或是列表，或是复杂map保存到对象中，这里的数据都是整存整取
 ...
 * 4.set:和队列类似，但支持去重，却是无序的 5.Sorted set：与set比起来就是多了一个顺序
 * 
 * @author Justin Hu
 */
public interface CacheManager {
	 ...
	 * 
	 * <p>
	 * If the key does not exist or contains a value of a wrong type, set the key to
	 * the value of "0" before to perform the increment operation
	 * </p>
	 * 
	 * <p>
	 * add by jim
	 ...
	 *            After the timeout the key will be automatically deleted by the
	 *            server.<br>
	 *            A key with an associated timeout is said to be volatile in Redis
	 *            terminology.
	 ...
	 * 如果验证通过后需要将本次调用添加进去 基于redis集合特性实现
	 * 
	 * @param key
	 * @param limit
	 *            限制次数
	...
	/**
	 * 这个方法也是运行脚本，但和使用hashkey的方法比较其是直接运行脚本
	 * 
	 * @param script
	 * @param key
	 * @param value
