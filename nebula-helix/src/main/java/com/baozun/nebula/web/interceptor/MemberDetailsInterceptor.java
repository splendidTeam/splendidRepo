...
public class MemberDetailsInterceptor extends HandlerInterceptorAdapter implements ServletContextAware{
    ...
    protected boolean doValidateNeedLogin(HttpServletRequest request,HttpServletResponse response,NeedLogin needLogin) throws IOException{
     ...
     * @throws IOException
     */
    protected boolean doMemberStatusFlowProcess(MemberDetails memberDetails,HttpServletRequest request,HttpServletResponse response)
                    throws IOException{
        ...
        if (Validator.isNotNullOrEmpty(action)){
            LOG.info(
                            "[MEMBER_STATUS_FLOW_PROCESSOR]  memberId:{} ,status{} ,action{}",
    ...
    private String getReturnURL(HttpServletRequest request,NeedLogin needLogin){
        String returnUrl = needLogin.returnUrl();
        ...
        LOG.debug("Return URL:[{}] is found in NeedLogin", returnUrl);
        return returnUrl;
    }
