 ...
 * <ol>
 * <li>
 * {@link #returnCommit(MemberDetails memberDetails, BindingResult bindingResult, HttpServletRequest request, ReturnOderForm returnOrderForm, Model model)}
 * 退货提交</li>
 * 
 * </ol>
 * 
 ...
 * 
 * <ol>
 * <li>{@link #showReturnDetail(String isTrack, String orderId, Model model)}
 * 退货详情</li>
 * </ol>
 * 
 * 
...
public class NebulaOrderReturnController extends BaseController {
	...
	private static final Logger LOGGER = LoggerFactory
			.getLogger(NebulaOrderReturnController.class);
	...
	public NebulaReturnResult commitReturn(
		...
		returnApplicationNormalValidator.validate(returnOrderForm,
				bindingResult);
		...
		SalesOrderCommand salesOrder = orderManager.findOrderById(
				Long.parseLong(returnOrderForm.getOrderId()), null);

		// 如果是换货，需要处理换货物流对象
		ReturnApplicationCommand returnCommand = returnApplicationResolver
				.toReturnApplicationCommand(memberDetails, returnOrderForm,
						salesOrder);

		returnCommand = soReturnApplicationManager
				.createReturnApplication(returnCommand, salesOrder);
	 ...
	 * 
	 */
	public String showReturnApplication(
			@LoginMember MemberDetails memberDetails,
			@RequestParam(value = "orderLineIds", required = true) List<Long> orderLineIds,
			Model model) {
			// 校验
		  Validate.notNull(memberDetails, "memberDetails can't be null!");
	 ...
	 * @return
	 */
	public String showReturnDetail(@LoginMember MemberDetails memberDetails,
			@RequestParam(value = "returnIds") List<Long> returnIds,
			Model model, HttpServletRequest request) {
		// 校验
		  Validate.notNull(memberDetails, "memberDetails can't be null!");
		  
		// 判断memberDetails和order id关系
			List<ReturnApplicationCommand> returnApplications = soReturnApplicationManager
					.findReturnApplicationCommandsByIds(returnIds);
			...
			List<ReturnApplicationViewCommand> returnApplicationViewCommands = soReturnApplicationManager
					.findReturnApplicationViewCommand(returnApplications);
