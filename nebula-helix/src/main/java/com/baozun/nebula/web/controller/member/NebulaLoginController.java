...
public class NebulaLoginController extends NebulaAbstractLoginController{
     ...
     */
    public NebulaReturnResult login(@ModelAttribute LoginForm loginForm,BindingResult bindingResult,HttpServletRequest request,HttpServletResponse response,Model model){
    ...
    protected boolean validateAutoLogin(LoginForm loginForm,MemberCommand memberCommand,HttpServletRequest request){
        ...
        if (isNotNullOrEmpty(loginName) && isNotNullOrEmpty(autoLoginPassword)){
            // 页面传来的密码必须和默认密码相同
     ...
     */
    protected void rememberMeProcess(MemberLoginViewCommand memberLoginViewCommand,HttpServletRequest request,HttpServletResponse response,Model model){
    ...
    protected void setAutoLoginPassword(HttpServletResponse response,LoginForm loginForm,MemberCommand memberCommand){
        try{
            setEncryptedInformationFromCookie(response, COOKIE_KEY_AUTO_LOGIN, generateAutoLoginPassword(loginForm, memberCommand), getRemberMeValidityPeriod());
        }catch (EncryptionException e){
            LOG.warn("Error when Settting Remeber Me information: {}", e);
        }
    }
     ...
     */
    private void setEncryptedInformationFromCookie(HttpServletResponse response,String name,String value,int maxAge) throws EncryptionException{
     ...
     */
    protected void doRememberMeProcess(LoginForm loginForm,MemberCommand memberCommand,HttpServletRequest request,HttpServletResponse response,Model model){
