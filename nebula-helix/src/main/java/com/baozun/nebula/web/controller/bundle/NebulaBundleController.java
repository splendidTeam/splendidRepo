...
public class NebulaBundleController extends NebulaAbstractBundleController {
	...
	public String showBundleList(@ModelAttribute("page") PageForm pageForm, HttpServletRequest request,
		...
		Pagination<BundleCommand> bundleCommandPage = nebulaBundleManager.findBundleCommandByPage(pageForm.getPage(),
				pageForm.getSorts());
	...
	protected List<BundleViewCommand> buildBundleViewCommandForPDP(List<BundleCommand> bundleCommands) {
		...
		for (int i = 0; i < result.size(); i++) {
			...
			command.setBundleElementViewCommands(
					buildBundleElementViewCommand(bundleCommands.get(i).getBundleElementCommands()));
		}
	...
	protected BundleDetailViewCommand buildBundleViewCommandForBundlePage(BundleCommand bundleCommand) {
		...
		if (itemBaseInfoViewCommand.getLifecycle() != 1) {
			LOG.info("Bundle error...bundleLifecycle is not active;Lifecycle:{} [{}]",
					itemBaseInfoViewCommand.getLifecycle(), new Date());
			return null;
		}
		...
		if (Validator.isNullOrEmpty(bundleElementViewCommand)) {
			LOG.info("Bundle error...bundleElementViewCommand is null;bundleId:{} [{}]", bundleCommand.getId(),
					new Date());
			return null;
		}
	 ...
	 */
	@Override
	protected List<BundleElementViewCommand> buildBundleElementViewCommand(
		...
		for (int i = 0; i < result.size(); i++) {
			...
			List<BundleItemViewCommand> bundleItemViewCommands = buildBundleItemViewCommand(
					bundleElementCommands.get(i).getItems());
	...
	protected List<BundleItemViewCommand> buildBundleItemViewCommand(List<BundleItemCommand> bundleItemCommands) {
		...
		for (int i = 0; i < result.size(); i++) {
			...
			// 加载商品销售属性
			ItemPropertyViewCommand itemPropertyViewCommand = buildItemPropertyViewCommand(
					buildProductBaseInfoViewCommand(itemId), buildItemImageViewCommand(itemId));
	...
	protected List<BundleSkuViewCommand> buildBundleSkuViewCommand(List<BundleSkuCommand> bundleSkuCommands) {
		...
		// List<SkuProperty> skuProperty =
		// sdkSkuManager.getSkuPros(sku.getProperties());
