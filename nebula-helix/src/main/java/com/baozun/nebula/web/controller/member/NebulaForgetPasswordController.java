...
public class NebulaForgetPasswordController extends BaseController {
    ...
    /**
     * <pre>判断验证方式是邮箱验证还是手机验证：邮箱验证：type=2； 手机验证：type=1 </pre>
     * @param type
     * @return String
    ...
    public String checkValidateCode(
		...
		if (forgetPasswordForm.getType() == 1 ) {
			if (VerifyResult.SUCESS == tokenManager.verifyToken(null, forgetPasswordForm.getMobile(),forgetPasswordForm.getSecurityCode())) {
				//	验证成功：则跳转到重置密码页面
			...
			if (VerifyResult.SUCESS == tokenManager.verifyToken(null, forgetPasswordForm.getEmail(),forgetPasswordForm.getSecurityCode())) {
				//	验证成功：则跳转到重置密码页面
				return VIEW_RESET_PASSWORD;
			} else {
				//	验证失败：则重新跳转到忘记密码页面，要求重新进行验证
    ...
    public String resetPassword(
		...
		//	前台传过来的密码解密后的密码
		String newPassword = decryptSensitiveDataEncryptedByJs(forgetPasswordForm.getNewPassword(), request);
		String confirmPassword = decryptSensitiveDataEncryptedByJs(forgetPasswordForm.getConfirmPassword(), request);
		...
		if (StringUtils.isNotEmpty(newPassword) && StringUtils.isNotEmpty(confirmPassword)) {
			...
			if (newPassword.equals(confirmPassword)) {
				...
				if (forgetPasswordForm.getType() == 2) {
					...
					MemberCommand memberCommand = sdkMemberManager.findMemberByLoginMobile(forgetPasswordForm.getMobile());
					memberId = memberCommand.getId();
					loginName = memberCommand.getLoginName();
				}
				...
				//	加密新密码
				String codePassword = EncryptUtil.getInstance().hash(newPassword, loginName);
				
				// 更新密码
				boolean resetPassword = sdkMemberManager.resetPasswd(memberId, codePassword);
				if (resetPassword) {
					// 修改密码成功，跳转至修改成功页面 
