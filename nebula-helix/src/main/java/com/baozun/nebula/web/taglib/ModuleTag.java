...
public class ModuleTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		if (applicationContext == null) {
			...
			sdkCmsModuleInstanceManager = (SdkCmsModuleInstanceManager) applicationContext
					.getBean("sdkCmsModuleInstanceManager");
		...
		try {
			/**
			 * 1、获取当前标签对应模块版本的缓存（静态map） 2、判断标签的模块是否在缓存时间段内 2.1、如果在就获取缓存模块版本的数据
			 * 2.1.1、先保存发布模块实例版本，将发布版本推送到发布表中 2.1.2、重新更新发布队列缓存
			 * 2.1.3、重新更新发布模块的内容到缓存中 2.2、不在就返回基础版本的数据
			 * 
			 */
			...
			if (Validator.isNotNullOrEmpty(cmsTemplateHtml)) {
				...
				logger.info("--------------------------------------> current version is " + currentVerison
						+ ", data is " + data);
				Optional<Map<String, List<CmsModuleInstanceVersionCommand>>> publishVersionsQueue = cache.getValue(CacheKeyConstant.CMS_MODULE_KEY);
				if (publishVersionsQueue.isPresent()) {
					boolean isBase = true;
					List<CmsModuleInstanceVersionCommand> versions = publishVersionsQueue.get().get(code);
					if (Validator.isNotNullOrEmpty(versions)) {
						Date now = new Date();
						for (CmsModuleInstanceVersionCommand version : versions) {
							// 判断当前时段是否有发布的新版本在队里发布队列里
							if (now.compareTo(version.getStart_time()) >= 0
									&& now.compareTo(version.getEnd_time()) <= 0) {
	...
	private GuavaAbstractLoadingCache<String, Optional<Map<String, List<CmsModuleInstanceVersionCommand>>>> cache = new GuavaAbstractLoadingCache<String, Optional<Map<String, List<CmsModuleInstanceVersionCommand>>>>(
		...
		protected Optional<Map<String, List<CmsModuleInstanceVersionCommand>>> fetchData(String key) {
			Map<String, List<CmsModuleInstanceVersionCommand>> publishVersionsQueue = cacheManager
					.getMapObject(CacheKeyConstant.CMS_MODULE_KEY, CacheKeyConstant.CMS_MODULE_VERSION_KEY);
