...
//			memberFrontendCommand.setIsaddgroup(0);//未加入组
			...
			if(null != memberCommand){
				retMemberCommand = (MemberFrontendCommand) ConvertUtils.convertTwoObject(new MemberFrontendCommand(),memberCommand);
	...
	private void saveMemberConductInfo(
			MemberFrontendCommand memberFrontendCommand) {
	...
	private void saveMemberPersonDataInfo(MemberFrontendCommand retMemberCommand) {
		...
		dataCommand.setBirthday(SdkDateUtils.parseStrToDate(retMemberCommand.getBirthday()));
	...
	@Override
	public Pagination<SalesOrderCommand> memberOrderList(Page page,
			 Long memberId) {
	...
	public MemberFrontendCommand loginSystem(MemberFrontendCommand memberFrontendCommand) {
		  MemberCommand memberCommand = sdkMemberManager.findMemberByLoginNameAndPasswd(memberFrontendCommand.getLoginName()
				  ,memberFrontendCommand.getPassword());
		  if(null != memberCommand){
			  //更新用户行为信息
	...
	public Pagination<CouponCommand> memberCouponList(Page page,Long memberId) {
		...
		Map<String,Object> paraMap=new HashMap<String, Object>();
		paraMap.put("memberId", memberId);
		Pagination<CouponCommand> paginationCouponCommand= sdkMemberManager.findCouponCommandList(page, sorts, paraMap);
	...
	public void deleteMemberFavorites(MemberFavorites memberFavorites) {
		...
		memberFavoritesCommand=(MemberFavoritesCommand) ConvertUtils.convertTwoObject(memberFavoritesCommand,memberFavorites);		
	...
	@Override
	public MemberFavorites findMemberFavoritesByMemberIdAndItemId(Long itemId,
			Long memberId, Long skuId) {
	...
	@Override
	public MemberPersonalDataCommand saveMemberPersonDataCommand(
	...
	@Override
	public Integer updateContactIsDefault(Long memberId, Long contactId,
			boolean isDefault) {
		Integer count = 0;
		//将会员所有的地址设置为非默认
	...
	public MemberDetails getMemberDetails(Member member) {
		...
		List<MemberGroupRelation> memberGroupRelations = sdkMemberManager.findMemberGroupRelationListByMemberId(member.getId());
		...
		return new MemberDetails(member.getLoginName(),member.getLoginEmail(),member.getLoginMobile(),
				member.getId(),nickname,groups,comboIds,realname);
	}
}
