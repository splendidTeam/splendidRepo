...
public abstract class MemberEmailManagerImpl implements MemberEmailManager {
    ...
    private static final Logger log = LoggerFactory
	    .getLogger(MemberManagerImpl.class);
	...
	// 获取用户登录注册信息
	MemberConductCommand conduct = memberConductDao
		.findMemberConductCommandById(member.getId());
	// 拼接内容
	StringBuffer emailContent = new StringBuffer();
	emailContent.append(member.getId()).append(member.getLoginEmail())
		.append(member.getType()).append(member.getSource())
		.append(conduct.getRegisterTime());
	// 加密字符串
	String checksum = EncryptUtil.getInstance().hash(
		emailContent.toString(), sendMailKey);
	// 拼接链接字符串并加密
	StringBuffer activeEmailUrl = new StringBuffer();
	try {
	    StringBuffer encryptParams = new StringBuffer();
	    encryptParams.append("member_id=").append(member.getId())
		    .append("&checksum=").append(checksum).append("&sendTime=")
		    .append(new Date().getTime());
	    String encrypt = EncryptUtil.getInstance().encrypt(
		    encryptParams.toString());
	    String path = getRegEmailValidPath(request);
	    activeEmailUrl.append(path).append("?registerComfirm=")
		    .append(EncryptUtil.getInstance().base64Encode(encrypt));
	} catch (Exception e) {
	    log.error("sendErr-----------------------: ", e);
	}
	...
	// 发送
	commonSendEmail(this, EmailConstants.EMAIL_REGISTER_VALIDATE,
		member.getLoginEmail(), dataMap);
    }
	...
	boolean flag = false;
	MemberPersonalData personalData = sdkMemberManager
		.findMemberPersonData(memberId);
	if (personalData != null
		&& StringUtils.equals(EmailConstants.EMAIL_ACTIVE_YES,
			personalData.getShort2())) {
	    flag = true;
	}
	return flag;
    }
    ...
    @Override
    @Transactional(readOnly = true)
    public void sendRegsiterSuccessEmail(String email,
	    MemberPersonalData personalData) {
	...
	// 发送
	commonSendEmail(this, EmailConstants.EMAIL_REGISTER_VALIDATE, email,
		dataMap);
     ...
     *            内容
     */
    public void commonSendEmail(Object object, String emailTemplate,
	    String email, Map<String, Object> dataMap) {
	if (Validator.isNullOrEmpty(email)) {
	    return;
	}
	EmailEvent emailEvent = new EmailEvent(object, email, emailTemplate,
		dataMap);
