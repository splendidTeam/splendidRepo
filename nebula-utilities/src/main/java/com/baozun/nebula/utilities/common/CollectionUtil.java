...
public class CollectionUtil {
     ...
     * @param curList中间参数
     */
	public static <T> void descartesT(List<List<T>> dimvalue, List<List<T>> result, int layer, List<T> curList) {
        if (layer < dimvalue.size() - 1) {
            if (dimvalue.get(layer).size() == 0) {
            	CollectionUtil.descartesT(dimvalue, result, layer + 1, curList);
            } else {
                for (int i = 0; i < dimvalue.get(layer).size(); i++) {
                    List<T> list = new ArrayList<T>(curList);
                    list.add(dimvalue.get(layer).get(i));
                    CollectionUtil.descartesT(dimvalue, result, layer + 1, list);
                }
            }
