...
public class RequestToCommand {
	private static final Logger logger = LoggerFactory
			.getLogger(RequestToCommand.class);
	...
	private void setParamValue(Map<String,String> keyMap,Map<String,String> valueMap, PaymentServiceReturnCommand paymentServiceReturnCommand){
		for(String key : valueMap.keySet()){
			try {
					...
					Method m = ReflectionUtil.getMethod(paymentServiceReturnCommand.getClass(), mapKey,
							String.class);
					if (m != null)
				...
				logger.error("Error when setting Param "
						+ key + "'s value with " + valueMap.get(key), e);
			}
		}
	}
